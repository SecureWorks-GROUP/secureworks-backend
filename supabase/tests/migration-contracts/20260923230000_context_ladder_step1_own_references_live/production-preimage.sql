-- The live production body of resolve_context_attribution (md5(prosrc) e6d5a8ec5ab942dff7f288d579d1b8f4),
-- as read from production 23 Sep 2026: the 20260911171000 body without its
-- comment lines. Installed by setup.sql so this case starts from production's
-- pre-image, not the repository's.
...
BEGIN
 IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='public.resolve_context_attribution(public.business_events)'::regprocedure)
  <>'e6d5a8ec5ab942dff7f288d579d1b8f4' THEN RAISE EXCEPTION 'L1 fixture: production pre-image is not byte-identical to the live body'; END IF;
END $$;
