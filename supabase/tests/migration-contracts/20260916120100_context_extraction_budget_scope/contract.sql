...
BEGIN
 ...
 -- Archived and holding jobs with fresh inbound words are still excluded.
 INSERT INTO public.business_events(job_id,match_method,direction,context_captured_at,event_at,payload) VALUES
 ...
 IF archived=ANY(ids) THEN RAISE EXCEPTION 'D4 archived job admitted'; END IF;
 ...
 IF (SELECT count(*) FROM public.context_extraction_events(archived,25))<>0 THEN RAISE EXCEPTION 'D4 archived job batch produced events'; END IF;
 ...
 -- Un-archiving the job with new inbound words makes it eligible again; the cap functions are untouched.
 UPDATE public.jobs SET status='accepted' WHERE id=archived;
 IF NOT EXISTS(SELECT 1 FROM public.context_extraction_candidates(400) c WHERE c.job_id=archived) THEN RAISE EXCEPTION 'D4 status is a live read'; END IF;
