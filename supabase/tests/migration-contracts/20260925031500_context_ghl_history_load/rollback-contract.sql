...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public'
  AND (p.proname LIKE 'context_ghl_history%' OR p.proname IN ('record_ghl_history_contact','capture_ghl_history_event','link_job_ghl_contact','reverse_ghl_contact_link')))
