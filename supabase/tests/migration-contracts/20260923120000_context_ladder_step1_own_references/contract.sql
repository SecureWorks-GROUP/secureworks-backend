...
BEGIN
 IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='public.resolve_context_attribution(public.business_events)'::regprocedure)
  <>'97c52abab1a1fa03e09c2174aca2fcad' THEN RAISE EXCEPTION 'L1: ladder body is not the L1 body'; END IF;
...
BEGIN
 IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='public.resolve_context_attribution(public.business_events)'::regprocedure)
  <>'97c52abab1a1fa03e09c2174aca2fcad' THEN RAISE EXCEPTION 'L1: re-apply changed the ladder body'; END IF;
END $$;
