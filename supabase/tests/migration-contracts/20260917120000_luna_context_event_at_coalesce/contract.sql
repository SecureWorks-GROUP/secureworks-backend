...
DECLARE org uuid:='00000000-0000-0000-0000-000000000001';
 ...
 fact uuid; occurred_time timestamptz:='2026-09-12 15:00:00+08';
 event_time timestamptz:='2026-09-10 12:00:00+08'; later_occurred timestamptz:='2026-09-15 12:00:00+08';
...
BEGIN
 ...
 INSERT INTO public.business_events(job_id,match_method,direction,payload,occurred_at)
  VALUES(j,'direct_job_id','inbound','{"body":"Crew are on site today. Delivery 14 September 2026."}',occurred_time)
 ...
 facts:=jsonb_build_array(
  ...
  jsonb_build_object('kind','pending_action','text','Delivery is arranged.','confidence',0.8,'source_event_ids',jsonb_build_array(e),'due_date','2026-09-14','evidence_excerpt','Delivery 14 September 2026.'));
 ...
 IF (SELECT event_date FROM public.job_temporary_context WHERE id=(result->'fact_ids'->>1)::uuid) IS DISTINCT FROM (occurred_time AT TIME ZONE 'Australia/Perth')::date
  OR (SELECT expires_at FROM public.job_temporary_context WHERE id=(result->'fact_ids'->>1)::uuid) IS DISTINCT FROM '2026-09-15 00:00+08'::timestamptz
