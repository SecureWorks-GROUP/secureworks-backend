...
BEGIN
 ...
 -- Backfill: Luna v1 rows read as luna with source-dated expiry; Haiku rows stay legacy.
 IF NOT EXISTS(SELECT 1 FROM public.job_context WHERE id='b3000000-0000-0000-0000-000000000003' AND trust='luna' AND extractor_version='context-luna-subscription:v1'
  AND event_date=((provenance->>'source_occurred_at')::timestamptz AT TIME ZONE 'Australia/Perth')::date
  AND expires_at=(provenance->>'source_occurred_at')::timestamptz+interval '504 hours'
  AND source_event_ids=ARRAY['b3000000-0000-0000-0000-00000000000e'::uuid])
 OR NOT EXISTS(SELECT 1 FROM public.current_job_context_facts WHERE id='b3000000-0000-0000-0000-000000000003' AND trust='luna')
 THEN RAISE EXCEPTION 'B3 v1 proposal backfill'; END IF;
 IF NOT EXISTS(SELECT 1 FROM public.job_temporary_context WHERE id='b3000000-0000-0000-0000-000000000005' AND trust='luna'
  AND expires_at=(provenance->>'source_occurred_at')::timestamptz+interval '168 hours')
 OR NOT EXISTS(SELECT 1 FROM public.current_job_context_facts WHERE id='b3000000-0000-0000-0000-000000000005')
 THEN RAISE EXCEPTION 'B3 v1 pending_action expiry moved from 24h TTL to event date'; END IF;
 IF NOT EXISTS(SELECT 1 FROM public.job_context WHERE id='b3000000-0000-0000-0000-000000000002' AND trust='legacy' AND extractor_version='context-fact-extractor:v1.5' AND event_date IS NULL AND expires_at IS NULL)
 THEN RAISE EXCEPTION 'B3 Haiku row must stay legacy and undated'; END IF;
 -- Operator override kinds survive byte-for-byte and stay readable as legacy.
 IF NOT EXISTS(SELECT 1 FROM public.job_context WHERE id='b3000000-0000-0000-0000-000000000004' AND kind='payment_agreement' AND lifecycle='current' AND trust='legacy')
 OR NOT EXISTS(SELECT 1 FROM public.current_job_context_facts WHERE id='b3000000-0000-0000-0000-000000000004' AND trust='legacy')
 THEN RAISE EXCEPTION 'B3 legacy override kind changed'; END IF;
 -- The nine-kind list is closed for Luna facts; the write-time trigger stamps trust.
 BEGIN
  INSERT INTO public.job_context(id,job_id,kind,value,provenance) VALUES(gen_random_uuid(),'b3000000-0000-0000-0000-000000000001','internal_instruction','{"text":"x"}','{"extractor":"context-luna-subscription:v1"}');
  RAISE EXCEPTION 'B3 luna row outside nine kinds accepted' USING ERRCODE='ZX001';
 EXCEPTION WHEN check_violation THEN NULL; END;
 INSERT INTO public.job_context(id,job_id,kind,value,provenance) VALUES('b3000000-0000-0000-0000-000000000006','b3000000-0000-0000-0000-000000000001','note','{"text":"v1 note"}','{"extractor":"context-luna-subscription:v1"}');
