...
-- business_events mirrors the production envelope (read 2026-09-16), including source_table/source_id.
CREATE TABLE public.business_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),sequence_number bigint,event_type text,source text,occurred_at timestamptz NOT NULL DEFAULT now(),recorded_at timestamptz DEFAULT now(),entity_type text,entity_id text,correlation_id uuid,causation_id uuid,job_id uuid REFERENCES public.jobs(id),payload jsonb NOT NULL DEFAULT '{}',metadata jsonb NOT NULL DEFAULT '{}',schema_version text,source_table text,source_id text,direction text,channel text,body_preview text,safe_summary text,body_pointer text,body_hash text,thread_key text,conversation_key text,contact_id text,match_status text,match_confidence numeric,match_method text,privacy_classification text,retention_class text);
