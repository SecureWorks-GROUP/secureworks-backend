...
/** Everything the handler needs from its environment; index.ts supplies the real ones. */
...
export async function handleGhlWebhook(
  ...
  try {
    ...
    try {
      ...
      // Not JSON: answered below with a receipt; the text is never logged.
    }

    decision = await decideAuth({
      headers: req.headers,
      rawBody,
      type: typeof body.type === "string" ? body.type : null,
      locationId: safeId(body.locationId),
      env: deps.env,
    });
    ...
    // The webhook body never sets the evidence row's job: a caller-supplied
    // job_id was a route for placing fake evidence on any job (audit C6).
    // Placement belongs to the attribution ladder (attribute_business_event).
    ...
    let captureOutcome: ReceiptOutcome = "event_created";
    ...
    if (type === "CallCompleted") {
      ...
      const _serviceJwt = deps.env("SUPABASE_SERVICE_ROLE_KEY") || "";
      ...
      try {
        deps.waitUntil?.(transcribeChain);
      } catch (_e) {
