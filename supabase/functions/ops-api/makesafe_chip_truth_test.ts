...
Deno.test("F5 sent-with-durable-pack-status: pack.status in sent-set -> true", () => {
  for (
    ...
    assertEquals(
      sentToBuilder(false, { status }),
      true,
      `pack.status=${status}`,
    );
    assertEquals(
      sentToBuilder(undefined, { status }),
      true,
      `undefined marker, pack.status=${status}`,
    );
  }
});
...
Deno.test("F5 completed-without-marker: completed card, no send record -> sent_to_builder false", () => {
  ...
  const invoice = {
    status: "AUTHORISED",
    invoice_date: "2026-07-03T00:00:00Z",
  };
  const docs = [
...
Deno.test("F5 completed-with-marker: same card WITH pack-sent marker -> sent_to_builder true", () => {
  ...
  const enriched = enrich(job, detail, [], null, invoice, docs, true, {
    status: "sent",
  });
...
Deno.test("F6 invoice-by-filename-fallback: untyped/'general' file named '...invoice...' -> true", () => {
  assertEquals(
    docBooleans([{ type: "general", file_name: "Tax Invoice 0776.pdf" }])
      .has_invoice_doc,
    true,
  );
  assertEquals(
    docBooleans([{ type: "", file_name: "invoice-0776.pdf" }]).has_invoice_doc,
    true,
  );
  assertEquals(
    docBooleans([{ file_name: "invoice.pdf" }]).has_invoice_doc,
    true,
  ); // type absent
});
...
Deno.test("F6 type wins: a meaningful non-invoice type is NOT spoofed by an 'invoice' filename", () => {
  ...
  const flags = docBooleans([{
    type: "makesafe_report",
    file_name: "make safe report - invoice appendix.pdf",
  }]);
...
Deno.test("F6 report-by-type and filename-fallback", () => {
  assertEquals(
    docBooleans([{ type: "makesafe_report", file_name: "REP-26438.pdf" }])
      .has_report_doc,
    true,
  );
  assertEquals(
    docBooleans([{
      type: "general",
      file_name: "SWMS-26438 Make Safe Report.pdf",
    }]).has_report_doc,
    true,
  );
  assertEquals(
    docBooleans([{ type: "invoice", file_name: "INV.pdf" }]).has_report_doc,
    false,
  );
});
...
Deno.test("F6 swms-by-type and job-number-prefix not matched by fallback", () => {
  assertEquals(
    docBooleans([{ type: "swms", file_name: "site-safety.pdf" }]).has_swms_doc,
    true,
  );
  ...
  assertEquals(
    docBooleans([{ type: "general", file_name: "SWMS method statement.pdf" }])
      .has_swms_doc,
    true,
  );
});
...
Deno.test("F6 has_wo stays type-only: a 'work order' filename on a general file does NOT count", () => {
  assertEquals(
    docBooleans([{ type: "work_order", file_name: "PO-55049.pdf" }]).has_wo,
    true,
  );
  assertEquals(
    docBooleans([{
  ...
  assertEquals(
    docBooleans([{ type: "general", file_name: "work order.pdf" }]).has_wo,
    false,
  );
});
