...
function resolve(
  ...
  if (st.table === "job_assignments") {
    ...
    if (st.select.startsWith("jobs:job_id(")) {
      return {
        data: page(joined.map(({ job }) => ({ jobs: { ...job } })), st),
        error: null,
      };
    }
  ...
  if (st.table === "calendar_events") {
    ...
    if (st.eq.org_id != null) {
      rows = rows.filter((e) => (e.org_id || TENANT_A) === st.eq.org_id);
    }
    if (st.eq.user_id != null) {
      rows = rows.filter((e) => e.user_id === st.eq.user_id);
    }
    if (st.neq.assignment_status != null) {
      rows = rows.filter((e) =>
        (e.assignment_status || "scheduled") !== st.neq.assignment_status
      );
    }
    if (st.lte.scheduled_date != null) {
      rows = rows.filter((e) => e.scheduled_date <= st.lte.scheduled_date);
    }
    ...
    for (const orFilter of st.ors) {
      ...
      rows = rows.filter((e) =>
        matchOr(e as unknown as Record<string, unknown>, orFilter.str)
      );
    }
    rows = rows.slice().sort((a, b) =>
  ...
  return {
    assignments: [
      ...
      },
      {
        ...
        status: "scheduled",
        scheduled_date: null,
        job_id: "job-unscheduled-decking",
      },
      {
        id: "a-tenant-b",
        user_id: "u-crew-b",
    ...
    jobs: [
      {
        ...
        type: "fencing",
        status: "invoiced",
        job_number: "SWF-26489",
      },
      {
        id: "job-old-makesafe",
        type: "makesafe",
        status: "archived",
        job_number: "SWMS-2200",
      },
      {
        id: "job-future-patio",
        type: "patio",
...
Deno.test("dispatcher Everyone carries the full range — years-old, future and unscheduled work all show", async () => {
  ...
  const primary = recorded.find((q) =>
    q.table === "job_assignments" && q.inCol == null && q.eq.user_id == null &&
    q.or == null
  );
  assertEquals(
    primary?.gte.scheduled_date,
    undefined,
    "no 30-day floor on the see-all feed",
  );
...
Deno.test("parity: a dispatcher's Everyone set contains the fencing manager's Everyone set", async () => {
  const fx = (): Fixtures => ({
    ...
    jobs: [
      ...
      },
      {
        ...
        job_number: "SWF-26378",
      },
      {
        id: "job-f-future",
        type: "fencing",
        status: "in_progress",
        job_number: "SWF-26850",
      },
    ],
  });
  ...
  assertEquals(
    henrySees.size,
    4,
    "the fencing lead sees the whole fencing history",
  );
  for (const jobId of henrySees) {
    assertEquals(
      marninSees.has(jobId),
      true,
      `the owner must also see ${jobId}`,
    );
  }
});
...
Deno.test("dispatcher Everyone pages the complete range without duplicates", async () => {
  ...
  assertEquals(
    ids.length,
    count,
    "every assignment past the 1000-row cap is present",
  );
  assertEquals(
    new Set(ids).size,
    count,
    "and none arrives twice across the page boundary",
  );
...
Deno.test("regression: an ordinary installer is untouched — own-only, still windowed", async () => {
  ...
  assertEquals(
    _managerBoardVerticals({
      isDispatcher: ALYX.isDispatcher,
      mode: "all",
      managedVerticals: [],
    }),
    [],
...
Deno.test("regression: a vertical manager's non-fencing lane keeps its rolling window", async () => {
  ...
  const recorded: RecordedQuery[] = [];
  const managerScope = _managerBoardVerticals({
    isDispatcher: nithin.isDispatcher,
    mode: "all",
    managedVerticals: ["patio"],
  });
  await myJobs(
...
Deno.test("the open make-safe pool is complete, not the newest 80", async () => {
  ...
  assertEquals(
    poolJobIds(grouped).length,
    total,
    "every allocatable make-safe is offered, not 80 of them",
  );
  assertEquals(
    new Set(poolJobIds(grouped)).size,
    total,
    "one job = one pool card",
  );
  const poolRead = recorded.find((q) =>
    q.table === "jobs" && q.or?.str.includes("type.eq.makesafe")
  );
...
Deno.test("make-safe detail reads: slim table scan, bounded full-row enrichment", async () => {
  ...
  const detailReads = recorded.filter((q) =>
    q.table === "makesafe_job_details"
  );
  const tableScan = detailReads.find((q) => q.inCol == null);
  assertEquals(
    tableScan?.select.includes("*"),
    false,
    "the whole-table scan selects no blob columns",
  );
  assertEquals(
    tableScan?.select.includes("substatus"),
    true,
    "just what the predicate reads",
  );
  ...
  assertEquals(
    fullRows.length > 0,
    true,
    "pool cards still get their full detail row",
  );
  for (const read of fullRows) {
    assertEquals(read.inCol, "job_id", "scoped to specific pool ids…");
    assertEquals(
      (read.inVals || []).length <= 25,
      true,
      "…and chunked for URL length",
    );
  }
  const contactReads = recorded.filter((q) => q.table === "job_contacts");
  assertEquals(
    contactReads.every((q) => (q.inVals || []).length <= 25),
    true,
    "contact backfill ids are chunked too",
  );
...
Deno.test("one job = one card no matter how many assignments it carries", async () => {
  const fixtures: Fixtures = {
    ...
    jobs: [{
      id: "job-rectification",
      type: "fencing",
      status: "schedule_install",
      job_number: "SWF-26624",
    }],
  };
  ...
  assertEquals(
    poolJobIds(grouped),
    [],
    "a job held by a live allocation is never offered as available",
  );
  const cards = nonPool(grouped).filter((a) =>
    a.jobs?.id === "job-rectification"
  );
  assertEquals(
    cards.length,
    6,
    "each real visit is still carried (the board needs per-week rows)",
  );
  assertEquals(
    new Set(cards.map((a) => a.id)).size,
    6,
...
function catalogFixtures(): Fixtures {
  return {
    ...
    jobs: [
      ...
      {
        id: "job-never-assigned",
        type: "fencing",
        status: "awaiting_supplier",
        job_number: "SWF-261099",
...
Deno.test("All tab: the company feed stops at the tenant and drops void/duplicate records", async () => {
  ...
  assertEquals(
    ids.includes("job-tenant-b"),
    false,
    "another tenant's jobs never appear",
  );
  assertEquals(
    ids.includes("job-dupe"),
    false,
    "a known duplicate record stays out — one job, one card",
  );
  assertEquals(
    ids.includes("job-cancelled"),
    true,
    "but cancelled work is real history and stays visible",
  );
...
Deno.test("All tab regression: ordinary crew keep the narrower active-jobs browse", async () => {
  ...
  assertEquals(
    seed?.eq.user_id,
    "u-alyx",
    "still seeded from their own assignments, as before",
  );
...
Deno.test("All tab: the company feed pages, and reports an honest total rather than a silent cap", async () => {
  ...
  assertEquals(
    capped.page_size,
    500,
    "an oversized page request is clamped, not honoured",
  );
});
...
Deno.test("search: a capped result keeps truncation honest without looping", async () => {
  ...
  const pages: Array<
    { jobs: Job[]; truncated: boolean; next_offset: number | null }
  > = [];
  let offset = 0;
  for (let i = 0; i < 10; i++) {
    ...
    assertEquals(
      page.jobs.length > 0,
      true,
      "a continuation never returns an empty page",
    );
...
Deno.test("All tab: external-ref matches are stable across pages and counted", async () => {
  ...
  const fixtures: Fixtures = {
    ...
    jobs: [
      ...
      },
      {
        ...
        status: "quoted",
        job_number: "SWF-1",
        client_name: "MLB25248 project one",
      ...
      },
      {
        ...
        status: "quoted",
        job_number: "SWF-2",
        client_name: "MLB25248 project two",
        created_at: "2026-01-01T00:00:00Z",
      },
    ],
  ...
  assertEquals(first.jobs.map((j: { id: string }) => j.id), [
    externalId,
    "00000000-0000-0000-0000-000000000001",
  ]);
  assertEquals(second.jobs.map((j: { id: string }) => j.id), [
    "00000000-0000-0000-0000-000000000002",
  ]);
  assertEquals(
    new Set([...first.jobs, ...second.jobs].map((j: { id: string }) => j.id))
      .size,
    3,
  );
  const baseReads = firstRecorded.filter((q) =>
    q.table === "jobs" && !q.head && q.or
  );
  assertEquals(
    baseReads[0]?.range,
    [0, 500],
    "search materializes one bounded result set before slicing",
  );
});
...
Deno.test("search: multiple external-ref matches respect page size", async () => {
  ...
  assertEquals(
    new Set(
      pages.flatMap((page) => page.jobs.map((job: { id: string }) => job.id)),
    ).size,
    3,
  );
});
...
Deno.test("search: exact job-number matches rank before newer weaker matches", async () => {
  ...
  assertEquals(res.jobs.map((job: { id: string }) => job.id), [
...
function calendarFixtures(): Fixtures {
  return {
    ...
    calendar: [
      {
        assignment_id: "c-2019",
        job_id: "job-2019",
        user_id: "u-crew1",
        job_type: "fencing",
        scheduled_date: "2019-04-02",
      },
      {
        assignment_id: "c-2024",
        job_id: "job-2024",
        user_id: "u-crew2",
        job_type: "makesafe",
        job_number: "SWMS-11",
...
Deno.test("calendar: the server adds no date floor — asking for 2019 returns 2019", async () => {
  ...
  const ids = res.events.map((e: { assignment_id: string }) => e.assignment_id)
    .sort();

  assertEquals(ids, ["c-2019", "c-2024", "c-decking", "c-patio"]);
  const query = recorded.find((q) => q.table === "calendar_events");
  assertEquals(
    query?.lte.scheduled_date,
    "2026-12-31",
    "the only date bound is the caller's own range",
  );
  assertEquals(
    query?.gte.scheduled_date,
    undefined,
    "no independent server-side floor",
  );
  assertEquals(
...
Deno.test("calendar: a dispatcher's Everyone view already carries every vertical", async () => {
  ...
  const types = new Set(
    res.events.map((e: { job_type: string }) => e.job_type),
  );
...
Deno.test("calendar regression: an ordinary installer is still held to their own rows", async () => {
  ...
  assertEquals(
    res.events.map((e: { assignment_id: string }) => e.assignment_id),
    ["c-patio"],
  );
  const query = recorded.find((q) => q.table === "calendar_events");
