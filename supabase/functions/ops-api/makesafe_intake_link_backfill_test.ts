...
Deno.test("recoverJobLinks: falls back to the draft extraction when the body is purged", () => {
  const { links, sources } = recoverJobLinks({
    ...
    email_body: null,
    draft_extraction: {
      portal_links: [{ label: "Roof report", url: "https://portal.mlb.example/x", kind: "roof_report" }],
    },
  });
...
Deno.test("summarizeLinkBackfill: a job that already has links is left untouched", () => {
  const report = summarizeLinkBackfill({
    jobs: [{
      job_id: "j-has-links",
      external_ref: "MLB-26011",
      report_type: "roof_report",
      current_external_links: [{ label: "Roof report", url: "https://x.example/r", kind: "roof_report" }],
      email_body: ROOF_EMAIL_BODY,
    }],
  });
...
Deno.test("summarizeLinkBackfill: archived report jobs are skipped unless includeArchived", () => {
  ...
  assertEquals(
    summarizeLinkBackfill({ jobs, includeArchived: true }).counts.patches_planned,
    1,
  );
});
