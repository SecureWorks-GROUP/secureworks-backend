...
import { extractPdfText, looksLikeText, PDF_TEXT_MIN_CHARS } from "./makesafe_pdf_text.ts";
...
async function flatePdf(content: string): Promise<Uint8Array> {
  ...
  let padded = content;
  while (deflated[deflated.length - 1] === 0x0a || deflated[deflated.length - 1] === 0x0d) {
    padded += " ";
...
const WO_CONTENT =
  "BT /F1 12 Tf 72 720 Td (Client: Jane Doe) Tj 0 -14 Td " +
...
Deno.test("FlateDecode text-layer PDF -> mode 'text' with the recovered fields (Deno DecompressionStream proof)", async () => {
  ...
  assert(r.charCount >= PDF_TEXT_MIN_CHARS, `expected >= ${PDF_TEXT_MIN_CHARS} chars, got ${r.charCount}`);
...
Deno.test("image-only PDF (no text operators) -> mode 'none'", async () => {
  ...
  const body = enc("\xff\xd8\xff\xe0 fake jpeg bytes here padding padding padding");
