...
function photoDataUrl(p: MakesafeReportPhoto): string | null {
  ...
  return `data:${fmt === 'PNG' ? 'image/png' : 'image/jpeg'};base64,${p.bytesBase64}`
}

// Resolve photo bytes: inline base64 wins; otherwise fetch the URL and base64 it.
async function resolvePhotoBytes(p: MakesafeReportPhoto): Promise<MakesafeReportPhoto | null> {
  ...
  try {
    ...
    return {
      ...
      contentType: resp.headers.get('content-type') || p.contentType || 'image/jpeg',
    }
  } catch {
    return null
  }
...
export async function renderMakesafeReportPdf(job: MakesafeReportJob): Promise<RenderedReport> {
  if (!job?.ref) throw new Error('renderMakesafeReportPdf: job.ref required')
  if (!job?.address) throw new Error('renderMakesafeReportPdf: job.address required')
  ...
  const footer = (): void => {
    ...
    doc.text(`SecureWorks Group | Page ${pageNo}`, PAGE_W / 2, PAGE_H - 9, { align: 'center' })
  }
  ...
  for (const [k, v] of rows) {
    if (y - rowH < 20) y = newPage()
    doc.setFillColor(...LIGHT)
    doc.setDrawColor(...RULE)
    doc.setLineWidth(0.2)
    doc.rect(MARGIN, y - rowH, CONTENT_W, rowH, 'FD')
    doc.setTextColor(...MID)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(7.5)
    doc.text(k.toUpperCase(), MARGIN + 4, y - 3.2)
    doc.setTextColor(...DARK)
    doc.setFontSize(8.4)
    doc.text(sanitiseText(v).slice(0, 105), MARGIN + 42, y - 3.2)
    y -= rowH
  }
  y -= 4
  ...
  const section = (title: string): number => {
    ...
    doc.rect(MARGIN, y - 8, CONTENT_W, 8, 'F')
    doc.setFillColor(...ORANGE)
    doc.rect(MARGIN, y - 8, 2.3, 8, 'F')
    doc.setTextColor(255, 255, 255)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(10.5)
    doc.text(title, MARGIN + 6, y - 2.6)
    return y - 13
  }
  ...
  if (resolved.length === 0) {
    ...
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(8.3)
    doc.text('No usable photo evidence was available for this report.', MARGIN, y)
  } else {
    const cellH = 190
    let idx = 0
    for (const p of resolved) {
      idx++
      if (y - cellH < 20) y = newPage()
      const dataUrl = photoDataUrl(p)
      doc.setDrawColor(...RULE)
      doc.setFillColor(255, 255, 255)
      doc.setLineWidth(0.2)
      doc.rect(MARGIN, y - cellH, CONTENT_W, cellH, 'FD')
      if (dataUrl) {
        try {
          const fmt = (p.contentType || '').toLowerCase().includes('png') ? 'PNG' : 'JPEG'
          ...
          doc.addImage(dataUrl, fmt, MARGIN + 3, y - cellH + 4, boxW, boxH, undefined, 'FAST')
        } catch {
          doc.setTextColor(...MID)
          doc.setFont('helvetica', 'normal')
          doc.setFontSize(8)
          doc.text(`Photo evidence ${idx} (could not embed)`, PAGE_W / 2, y - cellH / 2, { align: 'center' })
        }
      }
      doc.setTextColor(...MID)
      doc.setFont('helvetica', 'normal')
      doc.setFontSize(8)
      doc.text(`Photo evidence ${idx}`, PAGE_W / 2, y - 4, { align: 'center' })
      y -= cellH + 7
    }
  }
