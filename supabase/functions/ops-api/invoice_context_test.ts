    ...
    xero_invoices: [
      {
        ...
        job_id: JOB1,
        job_number: null,
        synced_at: "2026-09-11T03:30:00.000Z",
        line_items: [{
        ...
        raw_json: {
          Payments: [{
            Date: "/Date(1757462400000+0000)/",
            Amount: 1000,
            Reference: "part",
          }],
        },
        debt_classification: "genuine_debt",
        debt_classification_reason: "no reply",
        debt_classified_by: "DEBT",
        debt_classified_at: "2026-09-10T00:00:00.000Z",
      },
      // no job_id, reference names a job
      {
        org_id: ORG,
        invoice_type: "ACCREC",
        xero_invoice_id: INV2,
        xero_contact_id: "xc-2",
        contact_name: "Jorden Harris",
        invoice_number: "INV-1373",
        reference: "SWP-261180 FINBAL",
        status: "AUTHORISED",
        total: 3613.78,
        amount_due: 3613.78,
        amount_paid: 0,
        ...
        job_id: null,
        job_number: null,
        synced_at: "2026-09-09T03:30:00.000Z",
        line_items: [],
        raw_json: {},
      },
      // no job, contact has two jobs
      {
        org_id: ORG,
        invoice_type: "ACCREC",
        xero_invoice_id: INV3,
        xero_contact_id: "xc-3",
        contact_name: "Two Jobs Pty",
      ...
      // paid: not in the open population
      {
        org_id: ORG,
        invoice_type: "ACCREC",
        xero_invoice_id: INV5,
        xero_contact_id: "xc-1",
        contact_name: "Major Loss Builders",
        invoice_number: "INV-1416",
        reference: null,
        status: "PAID",
        total: 7579,
        amount_due: 0,
    ...
    jobs: [
      {
        ...
        type: "makesafe",
        status: "invoiced",
        client_name: "Patricia Quinn",
        client_phone: "0400",
        client_email: "p@x",
        site_address: "62 Lyrebird Way",
        site_suburb: "Thornlie",
        ghl_contact_id: "ghl-1",
        deposit_amount: null,
        deposit_at: null,
        pricing_json: { totalIncGST: 6237 },
        quoted_at: null,
        accepted_at: null,
      ...
      },
      {
        ...
        type: "patio",
        status: "completed",
        client_name: "Jorden Harris",
        ghl_contact_id: null,
        pricing_json: { total: 7227.56 },
      },
      {
        id: JOB3,
  ...
  return {
    ...
    from(table: string) {
      ...
      const run = () => {
        if (
          failing.has(table) ||
          (failFrom[table] !== undefined && callNo >= failFrom[table])
        ) {
...
function fakeConversation(tables: Tables) {
  return async (_client: any, body: { job_id: string; limit: number }) => {
    ...
    for (const e of tables.job_events ?? []) {
      if (e.job_id === body.job_id) {
        msgs.push({
          id: "note:1",
          channel: "note",
          direction: "internal",
          ...
          body: "internal note",
          preview: "internal note",
          source_system: "job_events",
          source_ref: "1",
        });
      }
    }
    for (const e of tables.business_events ?? []) {
      if (e.job_id === body.job_id) {
        msgs.push({
          id: "bev:1",
          channel: "email",
...
function deps(
  ...
  return {
    client: fakeClient(tables, failing, failFrom),
    orgId: ORG,
    getJobConversation: fakeConversation(tables),
    isCurrentContextFact,
    now: () => NOW,
  };
}
...
Deno.test("1. a linked invoice returns the complete picture with no blockers", async () => {
  ...
  assertEquals(out.conversation.messages.map((m: any) => m.source_ref), [
    "m1",
    "1",
    "1",
    "m2",
  ...
  assertEquals(out.bank.xero_payments, [{
    date: "2025-09-10",
    amount: 1000,
    reference: "part",
  }]);
...
Deno.test("3. a contact with several jobs is ambiguous with candidates; no route at all is no_job_linked", async () => {
  const t = baseTables();
  const amb = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1500" }),
    deps(t),
  );
  assertEquals(amb.link.status, "ambiguous");
  assertEquals(amb.link.candidates.map((c) => c.job_number).sort(), [
    "SWF-261300",
    "SWF-261301",
  ]);
  assertEquals(amb.job, null);
  assertEquals(
    amb.blockers.map((b) => b.code),
    ["job_link_ambiguous", "xero_stale"].filter((c) => c !== "xero_stale"),
  );
  assertEquals(amb.blockers[0].owner, "BOOKKEEPING");

  const none = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1501" }),
    deps(t),
  );
  assertEquals(none.link.status, "none");
  assertEquals(none.blockers.map((b) => `${b.code}:${b.owner}`), [
    "no_contact:BOOKKEEPING",
    "no_job_linked:BOOKKEEPING",
  ]);
...
Deno.test("3b. a single job through the Xero contact links; a stored job_number beats the reference", async () => {
  const t = baseTables();
  t.jobs = t.jobs.filter((j) =>
    j.id !== "a0000000-0000-4000-8000-000000000004"
  );
  const single = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1500" }),
    deps(t),
  );
  ...
  const stored = await invoiceContext(
...
Deno.test("4. one failing source is reported, the rest of the picture still returns", async () => {
  ...
  const out = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1419" }),
    deps(t, new Set(["current_job_context_facts", "payment_chase_logs"])),
  );
  assertEquals(out.sources.facts.ok, false);
  assertStringIncludes(
    out.sources.facts.error!,
    "current_job_context_facts unavailable",
  );
  ...
  assert(
    out.blockers.some((b) =>
      b.code === "facts_unreadable" && b.owner === "CIO"
    ),
  );
  assertEquals(out.blockers.some((b) => b.code === "facts_missing"), false);
  assertEquals(
    out.blockers.every((b) => !b.detail.includes("has not extracted")),
    true,
  );
...
Deno.test("5. bad input and unknown invoices are refused with a status", async () => {
  const t = baseTables();
  await assertRejects(
    () => invoiceContext(new URLSearchParams({}), deps(t)),
    InvoiceContextError,
    "invoice or xero_invoice_id is required",
  );
  await assertRejects(
    () =>
      invoiceContext(new URLSearchParams({ xero_invoice_id: "nope" }), deps(t)),
    InvoiceContextError,
    "must be a UUID",
  );
  await assertRejects(
    () =>
      invoiceContext(
        new URLSearchParams({ invoice: "INV-1419", mode: "everything" }),
        deps(t),
      ),
    InvoiceContextError,
    "mode must be card or full",
  );
  // LIKE wildcards are refused by name, so no invoice number can become a pattern
  await assertRejects(
    () => invoiceContext(new URLSearchParams({ invoice: "INV-%" }), deps(t)),
    InvoiceContextError,
    "wildcard characters % or _",
  );
  await assertRejects(
    () => invoiceContext(new URLSearchParams({ invoice: "%" }), deps(t)),
    InvoiceContextError,
    "wildcard characters % or _",
  );
  await assertRejects(
    () => invoiceContext(new URLSearchParams({ invoice: "INV_1419" }), deps(t)),
    InvoiceContextError,
    "wildcard characters % or _",
  );
  await assertRejects(
    () => invoiceContext(new URLSearchParams({ invoice: "INV 1419" }), deps(t)),
    InvoiceContextError,
  ...
  await assertRejects(
    () =>
      invoiceContext(new URLSearchParams({ invoice: "I".repeat(65) }), deps(t)),
    InvoiceContextError,
    "not a valid invoice number",
  );
  const err = await assertRejects(
    () => invoiceContext(new URLSearchParams({ invoice: "INV-9999" }), deps(t)),
    InvoiceContextError,
  );
  ...
  const paid = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1416" }),
    deps(t),
  );
  assertEquals(paid.invoice.status, "PAID");
  assertEquals(paid.job.other_open_invoices.map((o: any) => o.invoice_number), [
    "INV-1419",
  ]);
});
...
Deno.test("6. coverage counts the open population with the same rules", async () => {
  ...
  assertEquals(out.totals, {
    ...
    ghl_contact_known: 1,
    facts_present: 1,
    conversation_present: 1,
    xero_fresh: 3,
    complete: 1,
    distinct_linked_jobs: 2,
  });
  const byNo = Object.fromEntries(
    out.rows.map((r: any) => [r.invoice_number, r]),
  );
  assertEquals(byNo["INV-1419"].complete, true);
  assertEquals(byNo["INV-1419"].conversation_count, 3);
  assertEquals(byNo["INV-1419"].conversation_sources, {
    ghl_cache: 2,
    inbox: 0,
    business_events: 1,
    notes: 1,
  ...
  assertEquals(
    byNo["INV-1419"].last_client_message_at,
  ...
  assertEquals(byNo["INV-1373"].blockers, [
    "no_ghl_contact",
    "facts_missing",
  ...
  assertEquals(
    out.rows.some((r: any) => r.invoice_number === "INV-1416"),
    false,
  );

  const overdue = await debtContextCoverage(
    new URLSearchParams({ population: "overdue" }),
    deps(t),
  );
  assertEquals(overdue.totals.invoices, 3);
  await assertRejects(
    () =>
      debtContextCoverage(new URLSearchParams({ population: "all" }), deps(t)),
    InvoiceContextError,
  );
});
...
Deno.test("helpers: Xero dates, reference job numbers, queue detail", () => {
  ...
  assertEquals(
    jobNumberFromReference("Deposit swp-261180 patio"),
    "SWP-261180",
  );
...
Deno.test("luna_v2 stamps count as Luna; Haiku and instruction do not", async () => {
  ...
  const cases: Array<
    { name: string; luna: boolean; row: Record<string, unknown> }
  > = [
    {
      name: "v1",
      luna: true,
  ...
  const door = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1419" }),
    deps(t),
  );
  ...
  const doorCols = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1419" }),
    deps(tCols),
  );
  assertEquals(doorCols.facts.map((f: any) => f.id), ["v2-cols"]);
  assertEquals(doorCols.coverage.facts_present, true);
  const covCols = await debtContextCoverage(
    new URLSearchParams({}),
    deps(tCols),
  );
  const rowCols = covCols.rows.find((r: any) =>
    r.invoice_number === "INV-1419"
  )!;
  ...
  t.current_job_context_facts = [
    {
      ...
      value: "old haiku row",
      provenance: {
        extractor: "context-fact-extractor:v1.5",
...
Deno.test("9. an unreadable source is an unreadable blocker, never a missing one", async () => {
  ...
  const t1 = baseTables();
  const facts = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1419" }),
    deps(t1, new Set(["current_job_context_facts"])),
  );
  ...
  const t3 = baseTables();
  const conv = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1419" }),
    deps(t3, new Set(["inbox_events"])),
  );
  assert(
    conv.blockers.some((b) =>
      b.code === "conversation_unreadable" && b.owner === "CIO"
    ),
  );
  assertEquals(
    conv.blockers.some((b) => b.code === "conversation_missing"),
    false,
  );
  assertEquals(conv.coverage.conversation_present, false);
  assert(
    conv.warnings.some((w) =>
      w.startsWith("conversation_presence: read failed")
    ),
  );

  // job detail read down while the link itself resolved: job_read_failed (M2)
  const t4 = baseTables();
  const jobDown = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1419" }),
    deps(t4, undefined, { jobs: 2 }),
  );
  assert(
    jobDown.blockers.some((b) =>
      b.code === "job_read_failed" && b.owner === "CIO"
    ),
  );
  ...
  const cov = await debtContextCoverage(
    new URLSearchParams({}),
    deps(baseTables(), new Set(["current_job_context_facts", "inbox_events"])),
  );
  const row = cov.rows.find((r: any) => r.invoice_number === "INV-1419")!;
...
Deno.test("10. the door and the coverage read agree on every flag and blocker", async () => {
  ...
  for (const row of cov.rows) {
    const door = await invoiceContext(
      new URLSearchParams({ xero_invoice_id: row.xero_invoice_id }),
      deps(t),
    );
    assertEquals(
      door.link.status,
      row.link_status,
      `${row.invoice_number} link status`,
    );
    assertEquals(
      door.coverage.job_linked,
      Boolean(row.job_id),
      `${row.invoice_number} job_linked`,
    );
    assertEquals(
      door.coverage.contact_known,
      row.contact_known,
      `${row.invoice_number} contact_known`,
    );
    assertEquals(
      door.coverage.facts_present,
      (row.facts_count ?? 0) > 0,
      `${row.invoice_number} facts_present`,
    );
    assertEquals(
      door.coverage.conversation_present,
      (row.conversation_count ?? 0) > 0,
      `${row.invoice_number} conversation_present`,
    );
    assertEquals(
      door.coverage.xero_fresh,
      row.xero_fresh,
      `${row.invoice_number} xero_fresh`,
    );
    assertEquals(
    ...
    assertEquals(
      door.link.candidates.length > 0,
      door.link.status === "ambiguous",
      `${row.invoice_number} candidates`,
    );
  }
});
...
Deno.test("11. a job number that matches nothing is named in the blocker, not in candidates", async () => {
  ...
  const ghost = await invoiceContext(
    new URLSearchParams({ invoice: "INV-1419" }),
    deps(t2),
  );
