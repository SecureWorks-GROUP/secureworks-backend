...
function attentionFor(
  row: Required<Pick<MakesafeReconcileRow, "id" | "job_number">> &
  ...
  if (terminalProtected && before !== derived) {
    ...
    message =
      `Need you to choose which attendance cycle owns the ambiguous evidence; ${sentence(cycle.recovery_instruction)}`;
    evidenceRefs = cycle.evidence_refs;
  } else if (family) {
    code = "family_rule_ruling";
    message =
      `Need you to decide the completion rule for this job family; ${sentence(family.recovery_instruction)}`;
    evidenceRefs = family.evidence_refs;
  } else if (intake) {
    code = "intake_authority_ruling";
    message =
      `Need you to decide which intake instruction owns this job; ${sentence(intake.recovery_instruction)}`;
    ...
    message =
      `Need you to assign the state-evidence repair before trusting this card: ${sentence(repair)}`;
    evidenceRefs = [
      ...(input?.evidence_refs || []),
...
function resolvedAttention(
  ...
  return {
    ...
    message: "The fact-derived status is now trustworthy; no Captain action remains.",
    since: state.computed_at,
