...
export function isDoorLunaFact(
...
function daysBetween(fromIso: string | null, to: Date): number | null {
  ...
  const from = Date.parse(
    fromIso.length === 10 ? `${fromIso}T00:00:00Z` : fromIso,
  );
...
export function jobNumberFromReference(
...
async function pageThrough(
  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    const data = unwrap(
      await build().order(orderColumn, { ascending: true }).range(
        from,
        from + PAGE_SIZE - 1,
      ),
    ) || [];
  ...
  for (const inv of invoices) {
      ...
      links.set(
        ...
        j
          ...
          : {
            ...none(),
            hint:
              `the invoice's job_id ${inv.job_id} points at a job that does not exist`,
          },
      );
      continue;
    }
    const stored = str(inv.job_number)?.toUpperCase();
    const fromRef = stored
      ? null
      : jobNumberFromReference(inv.reference, inv.invoice_number);
    ...
    if (n) {
      ...
      if (j) {
        links.set(key, {
          status: "linked",
          method: stored ? "invoice.job_number" : "reference_job_number",
          job_id: j.id,
          job_number: j.job_number ?? null,
          candidates: [],
        });
      ...
      links.set(key, {
        ...none(),
        hint: `job number ${n} in the invoice does not match any job`,
      });
      continue;
    }
    if (inv.xero_contact_id) {
      ...
      if (found.size > 1) {
        links.set(key, {
          ...
          job_id: null,
          job_number: null,
          candidates: [...found.values()].map((j) => ({
            job_id: j.id,
            job_number: j.job_number ?? null,
            status: j.status ?? null,
            why: "same Xero contact",
          })),
        });
        continue;
      }
...
async function factsCountByJob(
  ...
  jobIds: string[],
  warnings: string[],
): Promise<{ counts: Map<string, number>; status: SourceStatus }> {
    ...
    for (const ids of chunk(jobIds)) {
      rows.push(
        ...await pageThrough(
          ...
          () =>
            deps.client.from("current_job_context_facts")
              .select(
                "id, job_id, kind, provenance, expires_at, _context_store, extractor_version, trust",
              ).in("job_id", ids),
          warnings,
        ),
      );
...
async function conversationCountsByJob(
  client: any,
  jobs: Array<{ id: string; ghl_contact_id: string | null }>,
  warnings: string[],
): Promise<{ counts: Map<string, ConversationCounts>; status: SourceStatus }> {
  ...
  for (const j of jobs) {
    if (j.ghl_contact_id) ghlToJob.set(j.ghl_contact_id, j.id);
  }
  ...
  try {
    ...
    const apply = (row: any, jobId: string) => {
      if (
        !jobId ||
        seen.has(`${jobId}:${row.contact_id ?? ""}:${row.job_id ?? ""}`)
      ) return;
    ...
    for (const ids of chunk([...ghlToJob.keys()])) {
      const data = unwrap(
        await client.from("ghl_conversation_cache").select(GHL_COLS).in(
          "contact_id",
          ids,
        ),
      );
      for (const row of data || []) {
        apply(row, ghlToJob.get(row.contact_id) ?? row.job_id);
      }
    }
    for (const ids of chunk(jobIds)) {
      const data = unwrap(
    ...
    errors.push(`ghl_cache: ${e instanceof Error ? e.message : String(e)}`);
  }
  ...
  try {
    for (const ids of chunk(jobIds)) {
      const rows = await pageThrough(
        "inbox_events",
        () =>
          client.from("inbox_events").select("id, job_id, received_at").in(
            "job_id",
            ids,
          ),
        warnings,
      ...
      for (const row of rows) {
        ...
        if (
          ...
          (!c.last_client_message_at ||
            row.received_at > c.last_client_message_at)
        ) c.last_client_message_at = row.received_at;
      }
    }
  } catch (e) {
  ...
  try {
    for (const ids of chunk(jobIds)) {
      const rows = await pageThrough(
        "job_events",
        () =>
          client.from("job_events").select("id, job_id").eq(
            "event_type",
            "note",
          ).in("job_id", ids),
        warnings,
    ...
    errors.push(`job_events: ${e instanceof Error ? e.message : String(e)}`);
  }
    ...
    for (const ids of chunk(jobIds)) {
      ...
      for (const row of rows) {
        ...
        const inbound = String(row.event_type).endsWith("_in") ||
          row.event_type === "client.reply";
        if (
          inbound && row.occurred_at &&
          (!c.last_client_message_at ||
            row.occurred_at > c.last_client_message_at)
        ) c.last_client_message_at = row.occurred_at;
      }
    }
  } catch (e) {
    errors.push(
      `business_events: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
  ...
  return {
    ...
    status: errors.length
      ? { ok: false, count: counts.size, error: errors.join("; ") }
      : { ok: true, count: counts.size },
  };
}
...
const INVOICE_COLS =
  "id, xero_invoice_id, xero_contact_id, contact_name, invoice_number, reference, invoice_type, status, total, amount_due, amount_paid, invoice_date, due_date, fully_paid_on, line_items, raw_json, job_id, job_number, synced_at, debt_classification, debt_classification_reason, debt_classified_by, debt_classified_at";
...
function invoiceView(inv: any, now: Date) {
  ...
  return {
    ...
    line_items: lineItems.slice(0, 50).map((li: any) => ({
      ...
      quantity: num(li?.Quantity ?? li?.quantity),
      unit_amount: num(li?.UnitAmount ?? li?.unit_amount),
      line_amount: num(li?.LineAmount ?? li?.line_amount),
    })),
    xero_synced_at: syncedAt,
    xero_cache_age_minutes: ageMinutes,
    classification: {
      class: inv.debt_classification ?? null,
      reason: inv.debt_classification_reason ?? null,
      by: inv.debt_classified_by ?? null,
      at: inv.debt_classified_at ?? null,
    },
  };
}

function xeroPayments(
  raw: any,
): Array<
  { date: string | null; amount: number | null; reference: string | null }
> {
...
export async function invoiceContext(
  ...
  if (!invoiceNo && !xeroId) {
    throw new InvoiceContextError("invoice or xero_invoice_id is required");
  }
  if (xeroId && !UUID.test(xeroId)) {
    throw new InvoiceContextError("xero_invoice_id must be a UUID");
  }
  if (invoiceNo) {
    ...
    if (/[%_]/.test(invoiceNo)) {
      throw new InvoiceContextError(
        "invoice must not contain the wildcard characters % or _",
      );
    }
    if (!INVOICE_NUMBER.test(invoiceNo)) {
      throw new InvoiceContextError("invoice is not a valid invoice number");
    }
  }
  const modeRaw = str(params.get("mode")) ?? "card";
  if (!(modeRaw in MODE_BOUNDS)) {
    throw new InvoiceContextError("mode must be card or full");
  }
  const mode = modeRaw as Mode;
  const clamp = (raw: string | null, cap: number) => {
    const n = num(raw);
    return n && n > 0 ? Math.min(Math.floor(n), cap) : cap;
  };
  const conversationLimit = clamp(
    params.get("conversation_limit"),
    MODE_BOUNDS[mode].conversation,
  );
  ...
  let q = client.from("xero_invoices").select(INVOICE_COLS).eq(
    "org_id",
    deps.orgId,
  ...
  q = xeroId
    ? q.eq("xero_invoice_id", xeroId)
    : q.ilike("invoice_number", escapeLikeLiteral(invoiceNo!));
  const invRows = unwrap(await q.limit(2));
  if (!invRows || invRows.length === 0) {
    throw new InvoiceContextError(
      `invoice not found: ${xeroId ?? invoiceNo}`,
      404,
      "invoice_not_found",
    );
  }
  if (invRows.length > 1) {
    throw new InvoiceContextError(
      `invoice number matches ${invRows.length} rows; pass xero_invoice_id`,
      409,
      "invoice_ambiguous",
    );
  }
  ...
  if (link.status === "linked" && link.job_id) {
    const jobRead = await safeRead(
      "jobs",
      async () =>
        unwrap(
          await client.from("jobs")
            .select(
              "id, job_number, type, status, client_name, client_phone, client_email, site_address, site_suburb, ghl_contact_id, deposit_amount, deposit_at, pricing_json, quoted_at, accepted_at, scheduled_at, completed_at, created_at",
            )
    ...
    if (jobRow) {
      const [
        ...
        safeRead(
          ...
          async () =>
            unwrap(
              await client.from("work_orders").select(
                "wo_number, trade_name, status, scheduled_date, completed_at",
              ).eq("job_id", jobRow.id).order("created_at", {
                ascending: false,
              }).limit(50),
            ),
        ),
        safeRead(
          ...
          async () =>
            unwrap(
              await client.from("council_submissions").select(
                "template_type, overall_status",
              ).eq("job_id", jobRow.id).order("updated_at", {
                ascending: false,
              }).limit(1),
            ),
        ),
        safeRead(
          ...
          async () =>
            unwrap(
              await client.from("current_job_context_facts")
                .select(
                  "id, job_id, kind, value, provenance, expires_at, _context_store, updated_at, extractor_version, trust",
                ).eq("job_id", jobRow.id).order("updated_at", {
                  ascending: false,
                }).limit(factsLimit * 2),
            ),
        ...
        safeRead(
          "conversation",
          async () =>
            (await deps.getJobConversation(client, {
              job_id: jobRow.id,
        ...
        safeRead(
          ...
          async () =>
            unwrap(
              await client.from("xero_invoices").select(
                "invoice_number, xero_invoice_id, amount_due, due_date, status",
              )
                .eq("org_id", deps.orgId).eq("invoice_type", "ACCREC").eq(
                  "job_id",
                  jobRow.id,
                ).in("status", OPEN_STATUSES).gt("amount_due", 0).neq(
                  "xero_invoice_id",
                  inv.xero_invoice_id,
                ).limit(20),
            ),
      ...
      facts = (factsRead.data || []).filter((row: any) =>
        isDoorLunaFact(deps, row, now.getTime())
      ).slice(0, factsLimit);
      if (
        facts.length < (factsRead.data || []).length &&
        (factsRead.data || []).length >= factsLimit * 2
      ) {
        warnings.push(
          `facts: read cap ${factsLimit * 2} reached; older facts not shown`,
        );
      }
      ...
      clientMessageCount = presence
        ? presence.ghl_cache + presence.inbox + presence.business_events
        : 0;
      ...
      job = {
        ...
        client_email: jobRow.client_email ?? null,
        site_address: jobRow.site_address ?? null,
        site_suburb: jobRow.site_suburb ?? null,
        ghl_contact_id: jobRow.ghl_contact_id ?? null,
        ...
        promised: {
          ...
          council: (council.data || [])[0]
            ? {
              template_type: council.data![0].template_type ?? null,
              overall_status: council.data![0].overall_status ?? null,
            }
            : null,
        },
        other_open_invoices: otherOpen.map((o: any) => ({
      ...
      blockers.push({
        ...
        owner: "BOOKKEEPING",
        detail:
          `Link points at job ${link.job_id} which no longer exists; link the invoice to the right job`,
      });
    } else {
      ...
      blockers.push({
        ...
        owner: "CIO",
        detail: `Job ${link.job_id} could not be read (${
          jobRead.status.error ?? "unknown error"
        }); the job picture is unknown`,
      });
    }
  }

  // 4. Chase log.
  const chaseRead = await safeRead(
    "payment_chase_logs",
    async () =>
      unwrap(
  ...
  const nextFollowUp =
    chaseRows.find((c: any) => c.follow_up_date && !c.follow_up_resolved)
      ?.follow_up_date ?? null;
  ...
  if (!contactKnown) {
    blockers.push({
      code: "no_contact",
      owner: "BOOKKEEPING",
      detail: "Invoice has no Xero contact",
    });
  }
  if (
    link.status === "none" && !blockers.some((b) => b.code === "no_job_linked")
  ) {
    blockers.push({
      code: "no_job_linked",
      owner: "BOOKKEEPING",
  ...
  if (link.status === "ambiguous") {
    blockers.push({
      code: "job_link_ambiguous",
      owner: "BOOKKEEPING",
      detail:
        `Contact has ${link.candidates.length} jobs; pick one (candidates listed)`,
    });
  }
  if (job && !job.ghl_contact_id) {
    blockers.push({
      code: "no_ghl_contact",
      owner: "CIO",
      detail: "Job has no GHL contact, so SMS and call history cannot attach",
    });
  }
  if (job && sources.facts?.ok === false) {
    blockers.push({
      ...
      owner: "CIO",
      detail: `The job_context read failed (${
        sources.facts.error ?? "unknown error"
      }); whether Luna has extracted this job is unknown`,
    });
  ...
  if (job && !conversationPresenceOk) {
    blockers.push({
      code: "conversation_unreadable",
      owner: "CIO",
      detail: `The stored message counts could not be read (${
        sources.conversation_presence?.error ?? "unknown error"
      }); whether this job has client messages is unknown`,
    });
  } else if (job && clientMessageCount === 0) {
    blockers.push({
      code: "conversation_missing",
      owner: "CIO",
      detail: "No stored client messages for this job in any source",
    });
  }
  const cacheAgeMinutes = inv.synced_at
    ? (now.getTime() - Date.parse(inv.synced_at)) / 60_000
    : null;
  const xeroFresh = cacheAgeMinutes !== null &&
    cacheAgeMinutes <= XERO_STALE_HOURS * 60;
  if (!xeroFresh) {
    blockers.push({
      code: "xero_stale",
      owner: "CIO",
      detail: cacheAgeMinutes === null
        ? "Invoice row has no sync time"
        : `Cache older than ${XERO_STALE_HOURS} h; balance may be wrong`,
    });
  }
  for (const [label, s] of Object.entries(sources)) {
    if (!s.ok) {
      warnings.push(`${label}: read failed (${s.error ?? "unknown"})`);
    }
  }
  if (job && sources.facts?.ok && facts.length === 0) {
    warnings.push("facts: 0 rows, extractor has not written for this job yet");
  }

  const lastClient =
    [...clientMessages].reverse().find((m: any) => m.direction === "inbound") ??
      null;
  const lastOutbound =
    [...clientMessages].reverse().find((m: any) =>
  ...
  return {
    ...
    invoice: {
      ...
      chase: {
        ...
        last: chaseRows[0]
          ? {
            at: chaseRows[0].created_at,
            method: chaseRows[0].method,
            outcome: chaseRows[0].outcome ?? null,
            notes: chaseRows[0].notes ?? null,
            by: chaseRows[0].chased_by ?? null,
          }
          : null,
    ...
    conversation: {
      ...
      last_client_message: brief(lastClient),
      last_outbound: brief(lastOutbound),
      sources: perSource,
    },
    bank: {
      xero_payments: xeroPayments(inv.raw_json),
      paid_in_bank_unreconciled: null,
    },
    blockers,
    coverage: {
...
export async function debtContextCoverage(
  params: URLSearchParams,
  deps: InvoiceContextDeps,
) {
  const now = (deps.now ?? (() => new Date()))();
  ...
  if (population !== "open" && population !== "overdue") {
    throw new InvoiceContextError("population must be open or overdue");
  }
  ...
  const sources: Record<string, SourceStatus> = {
    invoices: { ok: true, count: invoices.length },
  };
  const warnings: string[] = [];
  ...
  if (invoices.length >= PAGE_SIZE) {
    warnings.push(
      `invoices: read cap ${PAGE_SIZE} reached; the open population may be larger than this page`,
    );
  }
  ...
  const linkedIds = [
    ...new Set(
      [...links.values()].filter((l) => l.status === "linked" && l.job_id).map((
        l,
      ) => l.job_id!),
    ),
  ];
    ...
    for (const ids of chunk(linkedIds)) {
      rows.push(
        ...await pageThrough(
          "jobs",
          () =>
            client.from("jobs").select("id, job_number, ghl_contact_id").in(
              "id",
              ids,
            ),
          warnings,
  ...
  const [facts, queues, conv] = await Promise.all([
    ...
    conversationCountsByJob(
      ...
      linkedIds.map((id) => ({
        id,
        ghl_contact_id: jobs.get(id)?.ghl_contact_id ?? null,
      })),
      warnings,
    ),
  ]);
  ...
  for (const [label, st] of Object.entries(sources)) {
    if (!st.ok) {
      warnings.push(`${label}: read failed (${st.error ?? "unknown"})`);
    }
  }
  ...
  const totals = {
    ...
    ghl_contact_known: 0,
    facts_present: 0,
    conversation_present: 0,
    xero_fresh: 0,
    complete: 0,
    distinct_linked_jobs: linkedIds.length,
  };
  const rows = invoices.map((inv) => {
    const link = links.get(inv.xero_invoice_id) ??
      {
        status: "none",
        method: null,
        job_id: null,
    ...
    const conversationPresent = Boolean(jobId) && conversationOk &&
      clientCount > 0;
    const contactKnown = Boolean(inv.xero_contact_id);
    const ghlKnown = Boolean(job?.ghl_contact_id);
    const ageMinutes = inv.synced_at
      ? (now.getTime() - Date.parse(inv.synced_at)) / 60_000
      : null;
    const xeroFresh = ageMinutes !== null &&
      ageMinutes <= XERO_STALE_HOURS * 60;
    ...
    if (link.status === "linked" && !job) {
      blockers.push(jobsOk ? "no_job_linked" : "job_read_failed");
    }
    if (jobId && job && !ghlKnown) blockers.push("no_ghl_contact");
    if (jobId && !factsOk) blockers.push("facts_unreadable");
    else if (jobId && factsCount === 0) blockers.push("facts_missing");
    if (jobId && !conversationOk) blockers.push("conversation_unreadable");
    else if (jobId && clientCount === 0) blockers.push("conversation_missing");
    if (!xeroFresh) blockers.push("xero_stale");
    const dueDays = daysBetween(str(inv.due_date), now);
    const overdue = dueDays !== null && dueDays > 0;
    const complete = Boolean(jobId) && Boolean(job) && contactKnown &&
      factsPresent && conversationPresent && xeroFresh;
    totals.amount_due += num(inv.amount_due) ?? 0;
    if (overdue) totals.overdue += 1;
    if (link.status === "linked") totals.linked += 1;
    else if (link.status === "ambiguous") totals.ambiguous += 1;
    else totals.none += 1;
    ...
    return {
      ...
      job_id: jobId,
      job_number: link.job_number ?? job?.job_number ?? null,
      candidates: link.status === "ambiguous" ? link.candidates : [],
      contact_known: contactKnown,
      ghl_contact_known: jobId ? ghlKnown : null,
      facts_count: jobId && factsOk ? factsCount : null,
      conversation_count: jobId && conversationOk ? clientCount : null,
      conversation_sources: c
        ? {
          ghl_cache: c.ghl_cache,
          inbox: c.inbox,
          business_events: c.business_events,
          notes: c.job_events,
        }
        : null,
