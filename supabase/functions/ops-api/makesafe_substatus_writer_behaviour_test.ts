...
function makeRecordingClient(opts: FakeOpts = {}) {
      ...
      return {
        select(_cols?: string) {
          const readChain: any = {
            ...
            order: () => readChain,
            limit: () => readChain,
            maybeSingle: () => Promise.resolve(table === 'jobs' ? jobRead : detailRead),
            single: () => Promise.resolve(table === 'jobs' ? jobRead : detailRead),
  ...
  const detailWrites = () => writes.filter((w) => w.table === 'makesafe_job_details')
...
Deno.test('writer: each row shape issues exactly the request its origin issued before', async () => {
  const shapes: Array<[any, string[]]> = [
    ...
    [{ select: 'job_id', row: 'rows' }, ['eq(job_id=job-1)', 'select(job_id)', 'await']],
...
Deno.test('writer: narrow filters land between eq(job_id) and select — reattend compare-and-set', async () => {
  ...
  await _writeMakesafeSubstatus(
    ...
    internalEvidenceOrigin('reattend'),
    {
      ...
      row: 'maybeSingle',
      narrow: (q: any) =>
        q.eq('cycle_number', 1).eq('reattend_count', 0).is('report_received_at', null),
    },
  )
...
Deno.test('writer: a PostgREST error is RETURNED, not thrown', async () => {
  ...
  const { client } = makeRecordingClient({ writeResult: { data: null, error: fault } })
...
Deno.test('origin external: updateMakesafeSubstatus issues the same request as before', async () => {
  const { client, detailWrites, events } = makeRecordingClient({
    detailRead: { data: { substatus: 'company_contact_required' }, error: null },
  })
...
Deno.test('origin external: company_contacted_at is still stamped HERE, not by the writer', async () => {
  ...
  await _updateMakesafeSubstatus(viaAction.client, { job_id: 'job-1', substatus: 'company_contact_done' })
  assert(
    typeof viaAction.detailWrites()[0].payload.company_contacted_at === 'string',
...
Deno.test('origin external:details — a substatus edit goes through the writer, a plain edit does not', async () => {
  ...
  const res: any = await _updateMakesafeDetails(plain.client, { job_id: 'job-1', invoice_notes: 'n' })
  assertEquals(res.ok, true, 'a non-substatus details edit is not gated, before or after')
...
Deno.test('origin internal:portal_report_done — same payload keys and same request', async () => {
  ...
  assert(Array.isArray(w[0].payload.external_links), 'the append-only link merge survives')
