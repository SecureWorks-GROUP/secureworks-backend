...
export type MakesafeMoneyStageSubstatus =
  typeof MAKESAFE_MONEY_STAGE_SUBSTATUSES[number];
  ...
  return {
    ...
    refusal: strictEnabled && strictWouldRefuse
      ? {
        status: 403,
        message:
          `agent money-stage fence refused substatus='${nextSubstatus}' (caller=${origin.detail})`,
      }
      : null,
  };
}
...
export function observeMakesafeMoneyStageFence(
  ...
  const decision = evaluateMakesafeMoneyStageFence(
    nextSubstatus,
    origin,
    strictEnabled,
  );
