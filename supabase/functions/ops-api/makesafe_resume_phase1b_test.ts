...
function makeClient(seed: Rows, opts: FakeOpts = {}) {
  ...
  function builder(table: string) {
    ...
    const applyWrite = () => {
      if (mode === "insert") {
        if (opts.throwInsertTable === table) throw new Error(`forced insert throw on ${table}`);
      ...
      if (opts.failUpdateTable === table) {
        return { data: null, error: { message: `forced update error on ${table}` } };
      }
    ...
    const b: any = {
      ...
      order: () => b,
      limit: () => b,
      range: async (from: number, to: number) => ({ data: matched().slice(from, to + 1), error: null }),
      insert: (row: any) => { mode = "insert"; insertRow = Array.isArray(row) ? row[0] : row; return b; },
      upsert: (row: any) => { mode = "upsert"; insertRow = Array.isArray(row) ? row[0] : row; return b; },
      ...
      single: async () => {
        const m = matched();
        return { data: m[0] ? { ...m[0] } : null, error: m[0] ? null : { message: "no rows" } };
      },
      // Terminal await for both reads and writes.
      then: (resolve: (v: any) => any) => {
        if (mode === "select") return resolve({ data: matched().map((r) => ({ ...r })), error: null });
  ...
  return {
    ...
    storage: { from: () => ({ createSignedUrl: async (p: string) => ({ data: { signedUrl: `https://signed.test/${p}` }, error: null }) }) },
  };
}
...
function makeSeams(opts: {
  ...
  const counts = { getToken: 0, authorise: 0, fetchInvoicePdf: 0, attachDoc: 0, fetchReportPdf: 0, email: 0 };
  const deps = {
    getToken: async (_c: any) => { counts.getToken++; return { accessToken: "tok", tenantId: "ten" }; },
    xeroPost: async (path: string, _a: string, _t: string, body: any, _m?: string) => {
      // The send path only calls xeroPost to AUTHORISE the invoice.
      if (path.startsWith("/Invoices/") && body?.Invoices?.[0]?.Status === "AUTHORISED") counts.authorise++;
      return { Invoices: [{ Status: "AUTHORISED" }] };
    },
    fetchInvoicePdfBytes: async () => { counts.fetchInvoicePdf++; return new Uint8Array([1, 2, 3]); },
    attachDoc: async (_c: any, _b: any) => { counts.attachDoc++; return { document_id: "doc-x" }; },
    fetchReportPdfBytes: async () => { counts.fetchReportPdf++; return new Uint8Array([4, 5, 6]); },
    sendEmail: async (_payload: any) => {
      counts.email++;
      if (opts.emailMode === "throw") throw new Error("ECONNRESET after dispatch");
      if (opts.emailMode === "status5xx") return { ok: false, status: opts.emailStatus ?? 503, bodyText: async () => "upstream 503", json: async () => ({}) };
      if (opts.emailMode === "status4xx") return { ok: false, status: opts.emailStatus ?? 400, bodyText: async () => "bad payload", json: async () => ({}) };
      return { ok: true, status: 200, bodyText: async () => "", json: async () => ({ message_id: "msg-1" }) };
    },
  };
  return { counts, deps };
}

const MARKER = "MAKESAFE_PACK_SENT | main | INV-1 | to=workorders@b.com | 2026-06-17T00:00:00Z";
...
function readySeed(extra: Partial<Rows> = {}): Rows {
  return {
    jobs: [{ id: "job-1", job_number: "MS-1", type: "makesafe", status: "invoiced", client_name: "Jane" }],
    makesafe_job_details: [{
      job_id: "job-1", substatus: "admin_to_send_report", external_ref: "B-1", report_sent_at: null,
      makesafe_companies: { slug: "b", name: "Builder Co", report_recipient: "workorders@b.com" },
    }],
    job_documents: [
      { id: "d-rep", job_id: "job-1", type: "makesafe_report", file_name: "Make Safe Report B-1.pdf", storage_url: "https://docs.test/r.pdf", pdf_url: null },
      { id: "d-inv", job_id: "job-1", type: "invoice", file_name: "Xero Invoice INV-1.pdf", storage_url: "https://docs.test/i.pdf", pdf_url: null },
    ],
    xero_invoices: [{ xero_invoice_id: "xi-1", invoice_number: "INV-1", status: "DRAFT", reference: "B-1", job_id: "job-1", invoice_date: "2026-06-17" }],
    makesafe_report_packs: [{ id: "p-1", job_id: "job-1", pack_kind: "main", status: "drafted", report_doc_id: "d-rep", xero_invoice_id: "xi-1" }],
    job_events: [],
    business_events: [],
    ...extra,
  };
...
function packOf(client: any) {
  return client._tables.makesafe_report_packs.find((p: any) => p.job_id === "job-1");
}
function markerCount(client: any) {
  return client._tables.job_events.filter((e: any) =>
    e.event_type === "note" && String(e.detail_json?.text || "").startsWith("MAKESAFE_PACK_SENT | main")).length;
}
...
Deno.test("send_pack: happy path -> ONE authorise + ONE email + marker + close", async () => {
  ...
  const res: any = await _makesafeSendPackForTest(client, {
    job_id: "job-1", recipient_email: "workorders@b.com", subject: "Make Safe Completion B-1", html_body: "<p>done</p>",
  }, CTX, deps);
...
Deno.test("resume: authorised_not_sent re-call emails ONCE and does NOT re-authorise", async () => {
  ...
  assertEquals(res.sent, true);
  assertEquals(counts.authorise, 0, "must NOT re-authorise an already-authorised invoice");
...
Deno.test("resume: sent_marker_failed -> writes marker + closes, ZERO email + ZERO authorise", async () => {
  ...
  const { counts, deps } = makeSeams({ emailMode: "ok" });
  const res: any = await _makesafeSendPackForTest(client, {
    job_id: "job-1", recipient_email: "workorders@b.com", subject: "s", html_body: "<p>x</p>",
  }, CTX, deps);
...
Deno.test("resume: 'sending' without sending_resolution -> 409 (no email, no authorise)", async () => {
  ...
  seed.makesafe_report_packs[0].send_started_at = new Date(Date.now() - 10 * 60 * 1000).toISOString();
  const client = makeClient(seed);
  const { counts, deps } = makeSeams({ emailMode: "ok" });
  const res: any = await _makesafeSendPackForTest(client, {
    job_id: "job-1", recipient_email: "workorders@b.com", subject: "s", html_body: "<p>x</p>",
...
Deno.test("idempotency: marker present -> already_sent, NO email, NO authorise", async () => {
  const seed = readySeed();
  seed.job_events = [{ job_id: "job-1", event_type: "note", detail_json: { text: MARKER } }];
  const client = makeClient(seed);
  const { counts, deps } = makeSeams({ emailMode: "ok" });
  const res: any = await _makesafeSendPackForTest(client, {
    job_id: "job-1", recipient_email: "workorders@b.com", subject: "s", html_body: "<p>x</p>",
...
Deno.test("D-d: email DISPATCH THROWS (timeout/network) -> status='sending', NOT authorised_not_sent", async () => {
  ...
  let threw = false;
  try {
    await _makesafeSendPackForTest(client, {
      job_id: "job-1", recipient_email: "workorders@b.com", subject: "s", html_body: "<p>x</p>",
    }, CTX, deps);
  } catch (_e) { threw = true; }
  assert(threw, "an unknown-outcome dispatch throw should surface as an error");
  assertEquals(counts.authorise, 1, "the invoice WAS authorised before the send");
  assertEquals(counts.email, 1, "the send was attempted once (dispatched)");
  const p = packOf(client);
  assertEquals(p.status, "sending", "unknown outcome must land in 'sending' (NOT authorised_not_sent)");
  assert(p.status !== "authorised_not_sent", "must NEVER be authorised_not_sent (would re-offer a send => double-email)");
...
Deno.test("D-d: email returns 5xx (ambiguous) -> status='sending', NOT authorised_not_sent", async () => {
  const client = makeClient(readySeed());
  const { counts, deps } = makeSeams({ emailMode: "status5xx", emailStatus: 502 });
  try {
    await _makesafeSendPackForTest(client, {
      job_id: "job-1", recipient_email: "workorders@b.com", subject: "s", html_body: "<p>x</p>",
    }, CTX, deps);
  ...
  assertEquals(packOf(client).status, "sending", "a 5xx is ambiguous (may have dispatched) -> sending");
});
...
Deno.test("D-d: AMBIGUOUS 4xx (e.g. 408 timeout) -> 'sending', NOT authorised_not_sent", async () => {
  ...
  const client = makeClient(readySeed());
  const { counts, deps } = makeSeams({ emailMode: "status4xx", emailStatus: 408 });
  try {
    await _makesafeSendPackForTest(client, {
      job_id: "job-1", recipient_email: "workorders@b.com", subject: "s", html_body: "<p>x</p>",
    }, CTX, deps);
  ...
  assertEquals(counts.authorise, 1);
  assertEquals(packOf(client).status, "sending", "a 408 is ambiguous -> sending, never authorised_not_sent");
});
...
Deno.test("D-d: a marker-step throw AFTER a successful send -> sent_marker_failed (NOT authorised_not_sent)", async () => {
  ...
  const p = packOf(client);
  assert(p.status === "sent_marker_failed" || p.status === "sending",
    `post-send marker failure must be sent_marker_failed or sending, got ${p.status}`);
  assert(p.status !== "authorised_not_sent", "must NEVER be authorised_not_sent after a real send");
});
...
Deno.test("D-d contrast: a CLEAN 4xx pre-dispatch reject -> authorised_not_sent (re-send is safe)", async () => {
  ...
  const client = makeClient(readySeed());
  const { counts, deps } = makeSeams({ emailMode: "status4xx", emailStatus: 400 });
  try {
    await _makesafeSendPackForTest(client, {
      job_id: "job-1", recipient_email: "workorders@b.com", subject: "s", html_body: "<p>x</p>",
    }, CTX, deps);
  ...
  assertEquals(counts.authorise, 1);
  assertEquals(packOf(client).status, "authorised_not_sent", "a clean 4xx is provably not-sent -> authorised_not_sent");
});
...
function closeSeed(packStatus: string, withMarker: boolean): Rows {
  return {
    makesafe_report_packs: [{ id: "p-1", job_id: "job-1", pack_kind: "main", status: packStatus, sent_at: "2026-06-17T00:00:00Z" }],
    makesafe_job_details: [{ job_id: "job-1", substatus: "admin_to_send_report", report_sent_at: null }],
    job_events: withMarker ? [{ job_id: "job-1", event_type: "note", detail_json: { text: MARKER } }] : [],
  };
}
...
Deno.test("resume_close: sent_not_closed + marker -> close applied, pack->sent, ZERO email/authorise", async () => {
  ...
  const res: any = await _makesafeResumeCloseForTest(client, { job_id: "job-1" });
  assertEquals(res.resumed, true);
  ...
  assert(client._tables.makesafe_job_details[0].report_sent_at, "report_sent_at stamped");
});
...
Deno.test("resume_close: wrong status (authorised_not_sent) -> refused, pack NOT marked sent", async () => {
  ...
  assertEquals(packOf(client).status, "authorised_not_sent", "status untouched on refusal");
});
...
Deno.test("resume_close: marker ABSENT -> refused (no proof of send)", async () => {
  ...
  assertEquals(packOf(client).status, "sent_not_closed", "status untouched on refusal");
});
...
Deno.test("resume_close: a FAILED close update does NOT mark the pack sent", async () => {
  ...
  const client = makeClient(closeSeed("sent_not_closed", true), { failUpdateTable: "makesafe_job_details" });
  let threw = false;
  try {
    await _makesafeResumeCloseForTest(client, { job_id: "job-1" });
  } catch (_e) { threw = true; }
  assert(threw, "a failed close update must throw");
  assertEquals(packOf(client).status, "sent_not_closed", "pack must NOT be marked sent on a failed update");
});

Deno.test("resume_close: a ZERO-ROW close update does NOT mark the pack sent", async () => {
  const client = makeClient(closeSeed("sent_not_closed", true), { zeroRowUpdateTable: "makesafe_job_details" });
  let threw = false;
  try {
    await _makesafeResumeCloseForTest(client, { job_id: "job-1" });
  } catch (_e) { threw = true; }
  assert(threw, "a 0-row close update must throw (no job detail updated)");
  assertEquals(packOf(client).status, "sent_not_closed", "pack must NOT be marked sent on a 0-row update");
});
...
// ═════════════════════════════════════════════════════════════════
function resetSeed(packStatus: string, failedStep: string | null, withMarker: boolean): Rows {
  return {
    makesafe_report_packs: [{ id: "p-1", job_id: "job-1", pack_kind: "main", status: packStatus, failed_step: failedStep }],
    job_events: withMarker ? [{ job_id: "job-1", event_type: "note", detail_json: { text: MARKER } }] : [],
  };
}
...
Deno.test("reset: a PRE-SEND failed pack with NO marker resets to lockable 'drafted'", async () => {
  ...
  const res: any = await _makesafeResetFailedPackForTest(client, { job_id: "job-1" });
