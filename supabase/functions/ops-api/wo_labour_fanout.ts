...
export function _extractWoLabourEntries(item: any): { entries: WoLabourEntry[]; problems: WoLabourProblem[] } {
  const entries: WoLabourEntry[] = []
  const problems: WoLabourProblem[] = []
  const lines = Array.isArray(item?.wo_labour_lines) ? item.wo_labour_lines : []
  for (const raw of lines) {
    ...
    if (!name) {
      problems.push({ trade_name: '(unnamed)', reason: 'unnamed', amount, job_number: item?.job_number || null })
      continue
    }
    if (!(hours > 0) || !(rate > 0)) {
      problems.push({ trade_name: name, reason: 'incomplete', amount, job_number: item?.job_number || null })
      continue
    }
    entries.push({
      trade_name: name,
...
export function _cleanWoLabourLines(item: any): Array<{ trade_name: string; hours: number; rate: number; amount: number }> {
  const lines = Array.isArray(item?.wo_labour_lines) ? item.wo_labour_lines : []
  const out: Array<{ trade_name: string; hours: number; rate: number; amount: number }> = []
  for (const raw of lines) {
...
export function _woNetMismatch(item: any): { allocated: number; labourSum: number; expectedNet: number; claimedNet: number } | null {
...
export function _resolveWoLabourUsers(
  ...
  for (const entry of entries) {
    ...
    if (matches.length === 0) {
      problems.push({ trade_name: entry.trade_name, reason: 'unmatched', amount: entry.amount, job_number: entry.job_number })
      continue
    }
    if (matches.length > 1) {
      problems.push({ trade_name: entry.trade_name, reason: 'ambiguous', amount: entry.amount, job_number: entry.job_number })
      continue
    }
    const user = matches[0]
    if (String(user.id) === String(submitterId)) {
      problems.push({ trade_name: entry.trade_name, reason: 'self', amount: entry.amount, job_number: entry.job_number })
      continue
    }
...
export function _buildWoLabourPayoutInvoice(
  ...
  const lines = group.entries.map((e) => ({
    line_type: 'labour',
    description:
      `WO labour — ${[e.job_number, e.client_name].filter(Boolean).join(' — ') || 'job'} — ` +
    ...
    query_note: `${provenance}. Rate entered by ${opts.sourceTradeName} — office verifies before payment.`,
  }))
...
export function _woLabourProblemNote(problems: WoLabourProblem[]): string {
  ...
  return 'WO LABOUR: could not auto-pay ' + parts.join('; ') + '. The office must arrange these payment(s) manually.'
}
