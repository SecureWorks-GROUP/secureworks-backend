...
Deno.test("rejectIfPackSentMarker does NOT throw on an agent reply or plain note", () => {
  ...
  rejectIfPackSentMarker(
    "MAKESAFE_AGENT_REPLY | re-rendered in response to your note",
  );
...
Deno.test("buildAgentReplyBody prefixes the change description", () => {
  ...
  assertEquals(
    body,
    "MAKESAFE_AGENT_REPLY | widened the fence-hire line to 6 weeks",
  );
...
Deno.test("AGENT_REPLY_PREFIX and PACK_SENT_MARKER_PREFIX share no common prefix", () => {
  ...
  let i = 0;
  const min = Math.min(
    AGENT_REPLY_PREFIX.length,
    PACK_SENT_MARKER_PREFIX.length,
  );
  ...
  assertEquals(
    AGENT_REPLY_PREFIX.slice(0, i),
    PACK_SENT_MARKER_PREFIX.slice(0, i),
  );
