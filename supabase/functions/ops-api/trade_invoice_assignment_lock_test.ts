...
const ALYX_SIX = [
  { id: "202d174a-a084-455a-9de9-10b45efe8bc1", job: "SWMS-26841", invoiced_in: INV_PUSHED, hours_worked: null },
  { id: "92d7eeb0-f239-4bb9-989c-87d13ef75f6e", job: "SWMS-26835", invoiced_in: INV_PUSHED, hours_worked: null },
  { id: "bf821a9a-7265-4a8a-9098-574165095813", job: "SWMS-26836", invoiced_in: INV_PUSHED, hours_worked: null },
  { id: "fafa1293-af98-4e5b-bb79-e775d77cfd5f", job: "SWMS-26840", invoiced_in: INV_PUSHED, hours_worked: null },
  { id: "027bb8f4-e359-4140-a103-b21f71a18d15", job: "SWF-26613", invoiced_in: INV_PENDING, hours_worked: null },
  { id: "2d77301a-a21d-4c03-91aa-744a9344f524", job: "SWF-26609", invoiced_in: INV_PENDING, hours_worked: null },
];
...
Deno.test("clocked lane keeps a card released by a draft/ops-reject invoice", () => {
  ...
  assertEquals(kept.map((r) => r.id), ["a", "b"], "must not over-exclude released cards");
});
...
Deno.test("lock refuses BEFORE writing when every card is held (the 0-of-6 case)", () => {
  ...
  assertEquals(plan.missingIds.length, 0, "all six rows exist — this was never a missing-row bug");
});
...
Deno.test("lock refuses BEFORE writing on a PARTIAL match (the 02:56 half-claim)", () => {
  ...
  assertEquals(plan.ok, false, "a partial claim must be refused, not half-applied");
...
Deno.test("lock reclaims cards held by a RELEASED invoice, and only those", () => {
  ...
  assertEquals(plan.blockedIds, ["c"], "a released ref must not unlock a live-held card");
  assertEquals(plan.ok, false);
});
...
Deno.test("a dangling invoiced_in is blocked, never silently overwritten", () => {
  ...
  const plan = planAssignmentLock({
    expectedIds: ["a"],
    candidates: [{ id: "a", invoiced_in: "33333333-3333-3333-3333-333333333333" }],
    releasedInvoiceIds: [],
  });
  assertEquals(plan.ok, false);
...
Deno.test("index.ts clocked lane filters live-invoiced and never-clocked rows", () => {
  assertStringIncludes(INDEX, "selectUnlockedAssignments(asn || [], clockedLiveRefIds)");
...
Deno.test("index.ts never writes the CHECK-illegal 'failed' status to trade_invoices", () => {
  ...
  const offenders = [
    ...INDEX.matchAll(/from\('trade_invoices'\)\s*\.update\(\{[^}]*status:\s*'failed'/g),
  ];
  assertEquals(
    offenders.length,
    0,
...
Deno.test("the stamp-failure handler releases the invoice for retry", () => {
  ...
  const failHandlerAt = INDEX.indexOf("const failAssignmentStamp = async (stampMsg: string) => {");
