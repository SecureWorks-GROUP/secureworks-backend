    ...
    for (const path of ["user_mailbox_poll", "group_reader"] as const) {
      ...
      for (const row of pathRows) {
        ...
        assert(
          ...
          `${row.label}: classifier must not return a Promise`,
        );
...
Deno.test("EM0: a noreply sender and an urgent word keep their rules classes", () => {
  assertEquals(
    classifyEmail("noreply@council.example", "Building Permit issued", ""),
    {
  ...
  assertEquals(
    classifyEmail("client@example.com", "Urgent: gate left open", ""),
    {
