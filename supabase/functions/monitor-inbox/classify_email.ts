...
export function extractJobRef(
  ...
  if (po) {
    return po[0].toUpperCase().startsWith("PO-")
      ? po[0].toUpperCase()
      : `PO-${po[0].replace(/PO/i, "")}`;
  }
  return null;
}
...
export function classifyEmail(
  ...
  if (
    /noreply|no-reply|mailer-daemon|notifications?@|newsletter|unsubscribe/
      .test(fromLower) ||
