...
serve(async (req: Request) => {
  ...
  if (!GHL_WEBHOOK_SECRET) {
    console.error(
      "[ghl-webhook] GHL_WEBHOOK_SECRET not configured — rejecting request",
    );
  ...
  if (
    secret !== GHL_WEBHOOK_SECRET && secret !== `Bearer ${GHL_WEBHOOK_SECRET}`
  ) {
  ...
  try {
    ...
    console.log(
      "[ghl-webhook] Received payload type:",
      body.type || "unknown",
      "keys:",
...
function isStageChangeEvent(body: any): boolean {
  ...
  if (body.pipeline_stage_id && (body.opportunity_id || body.opportunityId)) {
    return true;
  }
  return false;
}
...
async function handleStageChange(sb: any, body: any) {
  ...
  const stageId = body.pipelineStageId || body.pipeline_stage_id ||
    body.opportunity?.pipeline_stage_id || "";
  const contactId = body.contactId || body.contact_id || body.contact?.id || "";

  console.log(
    `[ghl-webhook] Stage change: opp=${opportunityId} pipeline=${pipelineId} stage=${stageId} contact=${contactId}`,
  );
  ...
  if (pipelineId && !EXECUTION_PIPELINE_IDS.has(pipelineId)) {
    console.log(
      `[ghl-webhook] Pipeline ${pipelineId} is not an execution pipeline — skipping`,
    );
    return jsonResponse({
      success: true,
      action: "skipped",
      reason: "not_execution_pipeline",
    });
  }
  ...
  if (!newStatus) {
    console.log(
      `[ghl-webhook] No status mapping for stage ${stageId} — skipping`,
    );
    return jsonResponse({
      success: true,
      action: "skipped",
  ...
  if (!job) {
    console.log(
      `[ghl-webhook] No direct match for opp ${opportunityId} — attempting re-link`,
    );
  ...
  if (!job) {
    console.log(
      `[ghl-webhook] No job found for opportunity ${opportunityId} — skipping`,
    );
    return jsonResponse({
      success: true,
      action: "skipped",
  ...
  if (job.status === newStatus) {
    console.log(
      `[ghl-webhook] Job ${job.id} already ${newStatus} — skipping (anti-loop)`,
    );
    return jsonResponse({
      success: true,
      action: "skipped",
  ...
  if (newRank <= currentRank) {
    console.log(
      `[ghl-webhook] Conflict: ${job.status} (${currentRank}) → ${newStatus} (${newRank}) — backward move rejected`,
    );
    ...
    await sb.from("job_events").insert({
      ...
      detail_json: {
        ...
        message:
          `GHL backward move rejected: tried ${newStatus} but job is ${job.status}. Check GHL pipeline matches Ops Dashboard.`,
      },
    }).catch((e: any) =>
      console.error("[ghl-webhook] Failed to log conflict:", e)
    );
  ...
  try {
    const updateUrl =
      `${SUPABASE_URL}/functions/v1/ops-api?action=update_job_status`;
    ...
    console.log(
      `[ghl-webhook] Status updated: ${job.id} ${job.status} → ${newStatus}`,
      result,
    );
...
async function findJobByOpportunityId(sb: any, opportunityId: string) {
  const { data } = await sb
    .from("jobs")
    .select(
      "id, status, type, job_number, ghl_opportunity_id, ghl_contact_id, client_name",
    )
  ...
  if (customFields) {
    ...
    if (supabaseJobId) {
      ...
      if (data && data.length > 0) {
        job = data[0];
        console.log(
          `[ghl-webhook] Re-link matched by supabase_job_id: ${supabaseJobId}`,
        );
      }
    }
    ...
    if (!job && jobNumber) {
      ...
      if (data && data.length > 0) {
        job = data[0];
        console.log(
          `[ghl-webhook] Re-link matched by job_number: ${jobNumber}`,
        );
      }
    }
  }
  ...
  if (!job && contactId) {
    ...
    if (jobType) {
      ...
      if (data && data.length > 0) {
        job = data[0];
        console.log(
          `[ghl-webhook] Re-link matched by contact+type fallback: contact=${contactId} type=${jobType}`,
        );
      }
    }
  }
  ...
  if (job) {
    ...
    await sb.from("job_events").insert({
      ...
      detail_json: {
        ...
        message: `Re-linked from Sales to Execution pipeline (old: ${
          oldOppId || "none"
        })`,
      },
    }).catch((e: any) =>
      console.error("[ghl-webhook] Failed to log re-link:", e)
    );

    console.log(
      `[ghl-webhook] Re-linked job ${
        job.job_number || job.id
      }: ${oldOppId} → ${newOpportunityId}`,
    );
 ...
 */
async function fetchOpportunityCustomFields(
  opportunityId: string,
): Promise<Record<string, string> | null> {
  if (!GHL_API_TOKEN) {
    console.log(
      "[ghl-webhook] No GHL_API_TOKEN — cannot fetch opportunity custom fields for re-link",
    );
    return null;
  }
  ...
  try {
    ...
    if (!resp.ok) {
      console.log(
        `[ghl-webhook] GHL API returned ${resp.status} for opportunity ${opportunityId}`,
      );
      return null;
    }
    ...
    console.log(
      `[ghl-webhook] Opportunity ${opportunityId} custom fields:`,
      Object.keys(result).join(","),
    );
    return Object.keys(result).length > 0 ? result : null;
  } catch (e) {
    console.error(
      "[ghl-webhook] Failed to fetch opportunity custom fields:",
      (e as Error).message,
    );
    return null;
  }
...
async function handleFormSubmission(sb: any, body: any) {
  ...
  else if (mapped.utm_source === "google" && mapped.utm_medium === "cpc") {
    leadSource = "google_ads";
  } else if (mapped.utm_source) leadSource = mapped.utm_source;
  else if (body.source || body.lead_source) {
    leadSource = String(body.source || body.lead_source).toLowerCase();
  }
  ...
  if (matchErr) {
    console.error("contact_matches insert error:", matchErr.message);
  }
...
function mapGHLPayload(body: any): MappedData {
  ...
  const projectTypeRaw = find("project_type", "projectType", "service", "type")
    .toLowerCase();
  let type = "patio";
  if (projectTypeRaw.includes("fenc")) type = "fencing";
  else if (
    projectTypeRaw.includes("combo") || projectTypeRaw.includes("both")
  ) type = "combo";
  else if (
    projectTypeRaw.includes("patio") || projectTypeRaw.includes("pergola") ||
    projectTypeRaw.includes("carport")
  ) type = "patio";
  ...
  if (projectTypeRaw && projectTypeRaw !== type) {
    notesParts.push(`Project type: ${projectTypeRaw}`);
  }
  ...
  return {
    ...
    client_name:
      find("full_name", "fullName", "name", "firstName", "first_name") +
      (find("lastName", "last_name")
        ? " " + find("lastName", "last_name")
        : ""),
