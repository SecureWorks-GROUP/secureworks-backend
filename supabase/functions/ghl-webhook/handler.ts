...
export interface GhlWebhookDeps {
...
export async function handleGhlWebhook(
  ...
  const GHL_WEBHOOK_SECRET = deps.env('GHL_WEBHOOK_SECRET') || ''
  ...
  try {
    ...
    // Communication webhooks belong to ghl-webhook-receiver (slice C1c). They
    // are answered before the raw webhook_log write and never reach the
    // form-submission fallback below.
    if (isMessageWebhook(body)) return jsonResponse(messageWebhookAnswer())

    const sb = deps.createSupabase()
