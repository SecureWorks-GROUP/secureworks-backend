...
function siteFromScope(
  ...
  const address =
    pickFirstText(job, ["address", "site_address", "siteAddress"]) ??
...
function dedicatedMaterialsList(
  ...
  const candidates: Array<{ present: boolean; value: unknown }> = [
    {
      present: Object.prototype.hasOwnProperty.call(pricing, "materials_list"),
      value: pricing.materials_list,
    },
    {
      present: Object.prototype.hasOwnProperty.call(pricing, "bom"),
      value: pricing.bom,
    },
    {
      present: Object.prototype.hasOwnProperty.call(pricing, "materials"),
      value: pricing.materials,
    },
    {
      present: Object.prototype.hasOwnProperty.call(scope, "materials_list"),
      value: scope.materials_list,
    },
    {
      present: Object.prototype.hasOwnProperty.call(scope, "bom"),
      value: scope.bom,
    },
    {
      present: Object.prototype.hasOwnProperty.call(scope, "materials"),
    ...
    },
    {
      ...
      value: asObject(scope.job).materials_list,
    },
  ];
...
function isLabourOrCommissionLine(raw: Record<string, unknown>): boolean {
  ...
  const description = (asText(raw.description ?? raw.name ?? raw.label) ?? "")
    .toLowerCase();
...
> {
  ...
  asArray(pricing.runs).forEach((run, runIndex) => {
    ...
    const runKey = asText(
      runObj.run_id ?? runObj.id ?? runObj.run_label ?? runObj.run_name,
    ) ?? String(runIndex);
    asArray(runObj.items).forEach((item, i) => {
      ...
      out.push({
        raw,
        source: "pricing_run_items",
        hintId: `run:${runKey}:item:${i}`,
      });
    });
  });
  return out;
...
function quantityOf(raw: Record<string, unknown>): number | null {
  return finiteNumber(
    raw.quantity ?? raw.qty ?? raw.net_quantity ?? raw.netQuantity,
  );
}
...
function snapshotOverrideOf(raw: Record<string, unknown>): {
  ...
  const override = asObject(
    raw.price_snapshot_override ?? raw.priceSnapshotOverride,
  );
  const missing: string[] = [];
  if (override.approved !== true) {
    missing.push("price_snapshot_override.approved");
  }
  const approvedBy = [
    "approved_by",
    "approvedBy",
    ...
    .some((key) =>
      typeof override[key] === "string" && override[key].trim().length > 0
    );
  if (!approvedBy) {
...
function costExGst(pricing: Record<string, unknown> | null): number | null {
  ...
  const materials = finiteNumber(internal.cost) ??
    finiteNumber(pricing.materialCostEstimate);
  const labour = finiteNumber(internal.labour) ??
    finiteNumber(pricing.labourCostEstimate);
  const commission = finiteNumber(internal.commission) ??
    finiteNumber(pricing.commissionCostEstimate);
  const delivery = finiteNumber(internal.delivery) ??
    finiteNumber(internal.delivery_cost) ??
    finiteNumber(internal.deliveryCost) ??
    finiteNumber(pricing.deliveryCostEstimate) ??
    finiteNumber(pricing.delivery_cost) ??
    finiteNumber(pricing.deliveryCost) ??
    deliveryCostExGst(pricing);
  if (
    labour != null || materials != null || commission != null ||
    delivery != null
  ) {
    return roundCents(
      (labour ?? 0) + (materials ?? 0) + (commission ?? 0) + (delivery ?? 0),
    );
  }
  return null;
}
...
function reportLine(
  ...
  const snapshotDiffers = freezeSnapshot != null && lineSnapshot != null &&
    lineSnapshot !== freezeSnapshot;
...
export async function inspectFreezeOrderability(input: {
  ...
  const scope = coerceObject(revision.scope) ??
    parseJsonObject(revision.scope_canonical_text) ?? {};
  const pricing = coerceObject(revision.pricing) ??
    parseJsonObject(revision.pricing_canonical_text) ?? {};
  ...
  if (dedicated.present) {
    ...
    collectFallbackSellLines(pricing).forEach((item) => {
      lines.push(
        reportLine(item.raw, item.source, item.hintId, freezeSnapshot),
      );
    });
  }
  ...
  if (input.live) {
    ...
    const liveScopeHash = liveScope
      ? (await canonicalJsonAndHash(liveScope)).hash
      : null;
    const livePricingHash = livePricing
      ? (await canonicalJsonAndHash(livePricing)).hash
      : null;
    const storedScopeHash = asText(revision.scope_hash);
    const storedPricingHash = asText(revision.pricing_hash);
    if (storedScopeHash == null) {
      notComparableReasons.push("frozen_scope_hash_missing");
    } else if (!CONTENT_HASH_RE.test(storedScopeHash)) {
      notComparableReasons.push("frozen_scope_hash_malformed");
    }
    if (storedPricingHash == null) {
      notComparableReasons.push("frozen_pricing_hash_missing");
    } else if (!CONTENT_HASH_RE.test(storedPricingHash)) {
      notComparableReasons.push("frozen_pricing_hash_malformed");
    }
    const scope_hash_match =
      liveScope == null || liveScopeHash == null || storedScopeHash == null ||
        !CONTENT_HASH_RE.test(storedScopeHash)
        ? null
        : storedScopeHash === liveScopeHash;
    const pricing_hash_match = livePricing == null || livePricingHash == null ||
        storedPricingHash == null ||
  ...
  const verdict: FreezeOrderabilityVerdict = missing.length === 0
    ? "orderable"
    : "not_orderable";
...
export function summariseFreezeOrderability(
  ...
  for (const report of reports) {
    ...
    for (const item of report.missing) {
      missing_field_counts[item.field] =
        (missing_field_counts[item.field] ?? 0) + 1;
    }
  }
  ...
  for (const [field, count] of Object.entries(missing_field_counts)) {
    if (
      count > most ||
      (count === most && most_common_missing_field != null &&
        field < most_common_missing_field)
    ) {
      most = count;
...
type FreezeOrderabilityQueryChain = {
  select(columns: string): FreezeOrderabilityQueryChain;
  eq(column: string, value: unknown): FreezeOrderabilityQueryChain;
  maybeSingle(): Promise<{ data: unknown; error: unknown }>;
};

type FreezeOrderabilityClient = {
  from(table: string): FreezeOrderabilityQueryChain;
};
...
export async function inspectStoredFreezeOrderability(
  client: FreezeOrderabilityClient,
...
> {
  ...
  try {
    ...
    if (error) {
      return {
        ok: false,
        error: { code: "db_error", message: databaseErrorMessage(error) },
      };
    }
    ...
    return {
      ok: false,
      error: { code: "db_error", message: String((e as Error)?.message ?? e) },
    };
  }
  ...
  if (revisionRow.job_id) {
    try {
      ...
      if (data && typeof data === "object") {
        live = {
          scope_json: "scope_json" in data ? data.scope_json : undefined,
          pricing_json: "pricing_json" in data ? data.pricing_json : undefined,
        };
      }
    } catch (e) {
      return {
