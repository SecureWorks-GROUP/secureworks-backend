...
function completePricing(
  materials: Array<Record<string, unknown>> = completeMaterials,
) {
  return {
    source: "fence-designer",
    price_snapshot_id: SNAPSHOT_ID,
...
function missingFields(
  report: { missing: Array<{ field: string; line_id?: string }> },
): string[] {
  return report.missing.map((m) =>
    m.line_id ? `${m.field}@${m.line_id}` : m.field
  );
}
...
Deno.test("lineal-metres-only fence freeze is not_orderable and names sku, supplier, unit-cost", async () => {
  ...
  assert(
    ...
    `missing supplier, got ${fields.join(",")}`,
  );
  assert(
    fields.includes("unit_cost_ex_gst"),
    `missing unit_cost_ex_gst, got ${fields.join(",")}`,
  );
...
Deno.test("negative unit cost is not_orderable and names the affected line", async () => {
  ...
  assertEquals(report.verdict, "not_orderable");
  assertEquals(report.missing, [{
    field: "unit_cost_ex_gst",
    line_id: "mat-posts",
  }]);
  assertEquals(
    report.lines.find((line) => line.line_id === "mat-posts")?.unit_cost_ex_gst,
    false,
  );
});
...
Deno.test("partial line-level snapshot binding names the unbound line", async () => {
  ...
  assertEquals(
    report.lines.find((line) => line.line_id === "mat-posts")
      ?.price_snapshot_id,
    false,
  );
  for (const line of report.lines) {
...
Deno.test("freeze missing only its price-snapshot binding is not_orderable for that reason alone", async () => {
  ...
  assertEquals(report.verdict, "not_orderable");
  assertEquals(
    new Set(report.missing.map((m) => m.field)),
    new Set(["price_snapshot_id"]),
  );
  assertEquals(
...
Deno.test("perturbation: removing one line supplier flips verdict and names that line", async () => {
  ...
  const perturbed = await inspectFreezeOrderability({
    revision: await revisionFrom(
      siteScope,
      completePricing(perturbedMaterials),
    ),
  });
  assertEquals(perturbed.verdict, "not_orderable");
  assert(
    perturbed.missing.some((m) =>
      m.field === "supplier" && m.line_id === "mat-posts"
    ),
...
Deno.test("summarise counts orderable vs not and the most common missing field", async () => {
  ...
  assert(
    (summary.missing_field_counts[summary.most_common_missing_field] ?? 0) >= 2,
  );
});
...
class MockChain {
  ...
  maybeSingle(): Promise<{ data: Row | null; error: unknown }> {
    const rows = this.state[this.table].filter((r) =>
...
Deno.test("stored loader reads a frozen revision plus live job drift", async () => {
  ...
  const { canonical: scope_canonical_text, hash: scope_hash } =
    await canonicalJsonAndHash(siteScope);
  const { canonical: pricing_canonical_text, hash: pricing_hash } =
    await canonicalJsonAndHash(frozenPricing);
  ...
  const result = await inspectStoredFreezeOrderability(client, {
...
Deno.test("job-wide supplier is not inherited onto a line", async () => {
  ...
  assertEquals(report.verdict, "not_orderable");
  assert(
    report.missing.some((m) =>
      m.field === "supplier" && m.line_id === "mat-panels"
    ),
  );
});
...
Deno.test("identity gaps are named when freeze stamps are empty", async () => {
  ...
  for (
    ...
    assert(
      fields.includes(needed),
      `expected ${needed} in ${fields.join(",")}`,
    );
  }
});
