...
export function readCreateAllocationRef(
...
function parseOpportunityRecord(data: unknown): unknown | null {
  ...
  if (row.opportunity && typeof row.opportunity === "object") {
...
function nextSearchCursor(
  ...
  const lastRow = last && typeof last === "object"
...
export async function lookupOpportunityByAllocationRef(args: {
  ...
  for (let page = 1; page <= MAX_SEARCH_PAGES; page++) {
    ...
    const path = search.path.includes("status=")
      ? search.path
      : `${search.path}&status=all`;
...
export async function lookupAllocationOpportunityAction(args: {
  ...
  if (!ref) {
    return {
      status: 400,
      body: {
        error: "allocationRef is required",
        code: "allocation_ref_required",
      },
    };
  }
  const contactId = nonempty(args.contactId);
