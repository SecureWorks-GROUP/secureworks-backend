...
type AuthDecision =
  | { kind: 'allow'; mode: 'api_key' | 'jwt'; user: { id: string; email: string; role: string } | null }
  ...
  if (path === 'send' || path === 'send-invoice' || path === 'send-runs') {
    ...
    if (xApiKey && (xApiKey === validKey || xApiKey === serviceKey)) {
      ...
      try {
        ...
        if (error || !user) {
          return { kind: 'reject', response: new Response(JSON.stringify({ error: 'Session expired — please log in again' }), {
        ...
        return { kind: 'reject', response: new Response(JSON.stringify({ error: 'Authentication failed' }), {
          status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }) }
      }
    } else {
      return { kind: 'reject', response: new Response(JSON.stringify({ error: 'Unauthorized' }), {
        status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      }) }
    }
  ...
  return {
    auth: {
      getUser: (token: string) => {
        if (token === THROW_JWT) throw new Error('network unreachable (simulated)')
        if (token === VALID_JWT) {
          return Promise.resolve({ data: { user: { id: 'user-uuid-777', email: 'estimator@secureworksgroup.app' } }, error: null })
        }
    ...
    from: (_table: string) => ({
      select: (_c: string) => ({
        eq: (_col: string, _val: string) => ({
          maybeSingle: () => Promise.resolve({ data: { role: 'estimator' }, error: null }),
        }),
      }),
    }),
  }
...
function req(headers: Record<string, string>, path = 'send') {
  return new Request(`https://x.test/functions/v1/send-quote/${path}`, { method: 'POST', headers })
}
...
function captureAuthLog() {
  ...
  console.log = (...args: any[]) => {
    if (args[0] === '[send-quote] auth') { try { captured.push(JSON.parse(args[1])) } catch { /* ignore */ } }
  }
  return { captured, restore: () => { console.log = original } }
}
...
Deno.test("A1 — valid x-api-key header → allow, mode=api_key (unchanged path)", async () => {
  const d = await decideSendAuth(req({ 'x-api-key': ENV.SW_API_KEY }), makeAuthSb(), ENV, 'send')
...
Deno.test("A2 — Bearer master SW_API_KEY → allow, mode=api_key (unchanged path)", async () => {
  const d = await decideSendAuth(req({ authorization: `Bearer ${ENV.SW_API_KEY}` }), makeAuthSb(), ENV, 'send')
  assert(d.kind === 'allow')
  assertEquals(d.mode, 'api_key')
})
...
Deno.test("A3 — Bearer service-role key → allow, mode=api_key (unchanged path)", async () => {
  const d = await decideSendAuth(req({ authorization: `Bearer ${ENV.SUPABASE_SERVICE_ROLE_KEY}` }), makeAuthSb(), ENV, 'send-invoice')
  assert(d.kind === 'allow')
  assertEquals(d.mode, 'api_key')
})
...
Deno.test("A4 — valid user JWT bearer (scoping tool login) → allow, mode=jwt, user+role captured", async () => {
  ...
  try {
    const d = await decideSendAuth(req({ authorization: `Bearer ${VALID_JWT}` }), makeAuthSb(), ENV, 'send')
    assert(d.kind === 'allow')
    assertEquals(d.mode, 'jwt')
    assertEquals(d.user, { id: 'user-uuid-777', email: 'estimator@secureworksgroup.app', role: 'estimator' })
...
Deno.test("A4b — valid JWT with no users-row profile → allowed, role defaults to 'unknown' (not gated)", async () => {
  const sb = makeAuthSb()
  sb.from = (_t: string) => ({ select: (_c: string) => ({ eq: (_col: string, _v: string) => ({ maybeSingle: () => Promise.resolve({ data: null, error: null }) }) }) }) as any
  const d = await decideSendAuth(req({ authorization: `Bearer ${VALID_JWT}` }), sb, ENV, 'send')
  assert(d.kind === 'allow')
  assertEquals(d.mode, 'jwt')
...
Deno.test("A5 — expired/forged JWT → 401 'Session expired — please log in again'", async () => {
  const d = await decideSendAuth(req({ authorization: `Bearer ${EXPIRED_JWT}` }), makeAuthSb(), ENV, 'send')
  assert(d.kind === 'reject')
  assertEquals(d.response.status, 401)
  assertEquals((await d.response.json()).error, 'Session expired — please log in again')
})
...
Deno.test("A6 — getUser throws (transport failure) → 401 'Authentication failed'", async () => {
  const d = await decideSendAuth(req({ authorization: `Bearer ${THROW_JWT}` }), makeAuthSb(), ENV, 'send')
  assert(d.kind === 'reject')
  assertEquals(d.response.status, 401)
...
Deno.test("A8 — wrong x-api-key, no bearer → 401 'Unauthorized'", async () => {
  const d = await decideSendAuth(req({ 'x-api-key': 'not-the-key' }), makeAuthSb(), ENV, 'send')
  assert(d.kind === 'reject')
  assertEquals(d.response.status, 401)
  assertEquals((await d.response.json()).error, 'Unauthorized')
})
...
Deno.test("A10 — send / send-invoice / send-runs all reject anon and accept a user JWT", async () => {
  for (const p of ['send', 'send-invoice', 'send-runs']) {
    ...
    const jwt = await decideSendAuth(req({ authorization: `Bearer ${VALID_JWT}` }, p), makeAuthSb(), ENV, p)
