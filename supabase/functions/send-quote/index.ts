...
type RecordReleaseQuoteRevisionInput = {
  ...
  // ── Loop 3 / P2 V2 augmentation (optional) ──
  // When present, the helper builds the V2 envelope alongside the V1
  // minimal manifest and writes the V2 jsonb columns + internal_cost
  // columns into the same INSERT. Validator runs in mode='warn', so
  // this NEVER refuses the release. If the V2 build fails for any
  // reason (unknown adapter kind, etc.), we log [v2-augmentation-fail]
  // and INSERT V1-only — the V1 release-truth contract is preserved.
