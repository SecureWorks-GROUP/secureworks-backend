...
const INTELLIGENCE_TOOLS = [
  ...
  {
    name: 'cash_leak_detection',
    description: 'Identifies where money is being lost: underquoted jobs, unbilled delays, missing deposits, bad debt risk. Ranked by dollar impact.',
    input_schema: { type: 'object', properties: {} },
  },
  {
  ...
  },
  {
    ...
    description: 'Full AI intelligence assessment: risk level, health score (0-100), client quality score, margin forecast, KPI breach flags, things to know, next actions, AI summary. The complete picture for any job.',
    input_schema: {
      type: 'object',
      properties: {
        job_id: { type: 'string', description: 'Job UUID or job number (e.g. SWP-25042)' },
      },
      required: ['job_id'],
    },
  },
...
async function executeTool(name: string, input: any, view: string): Promise<{ result: any; needs_confirm?: boolean }> {
  switch (name) {
    ...
    case 'get_job_intelligence':
      return { result: await callReportingApi('job_intelligence', { job_id: input.job_id }) }
    ...
    case 'cash_leak_detection':
      return { result: await callReportingApi('cash_leak_detection') }
