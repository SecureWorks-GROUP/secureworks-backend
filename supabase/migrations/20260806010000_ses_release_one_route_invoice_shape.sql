-- SES release commit: accept the ruled one-route invoice shape.
--
-- Captain ruling 2026-08-06 (data/decisions/2026-08-06-roof-report-email-shape.md):
-- a roof-report card sends ONE email, to the group inbox, carrying the invoice.
-- commit_ses_release_revision_v1 previously accepted only 3-route universal or
-- 2-route AJS, so a ruled release RAISED at the database - the Captain could
-- authorise real money and only then be refused. This is the fifth and last
-- route-shape site for that one ruling.
--
-- WIDEN-ONLY. This is CREATE OR REPLACE, so "widens only" is a property of the
-- diff, not something the mechanism enforces. The body below was generated by
-- applying exactly three hunks to the live function body (verified byte-identical
-- to 20260804090000's, header formatting aside, by pg_get_functiondef on
-- production before writing this):
--
--   1. declare report_only_one, beside the existing universal / ajs_two flags
--   2. one ELSIF, mirroring the AJS branch: route_count = 1, kind 'invoice',
--      ordinal 0
--   3. one extra "AND NOT report_only_one" on the single refusal gate
--
-- Everything else is byte-identical: the type/emptiness guard, the advisory
-- lock, the content_hash idempotency and its "resolves to different content"
-- conflict path, both INSERT loops, and the REVOKE/GRANT. Every previously
-- accepted shape is accepted on identical conditions; every previously rejected
-- shape except the ruled one stays rejected. The full diff is in the PR.
--
-- The RAISE message is deliberately NOT updated. Changing it would be a fourth
-- delta and break the widen-only property this migration was approved on; it
-- only fires for shapes that are still refused. Retexting it is a follow-up.
