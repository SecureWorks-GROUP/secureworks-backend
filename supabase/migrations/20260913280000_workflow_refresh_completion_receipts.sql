-- Evidence-backed Refresh completion.
-- A driver must persist a scoped receipt before a run can become completed.
...
UPDATE public.workflow_refresh_drivers
   SET validator_key = CASE
     WHEN workflow = 'dispatch' THEN 'dispatch_source_v1'
     ELSE 'none'
   END
 WHERE validator_key IS NULL;

ALTER TABLE public.workflow_refresh_drivers
  ALTER COLUMN validator_key SET DEFAULT 'none',
  ALTER COLUMN validator_key SET NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conrelid = 'public.workflow_refresh_drivers'::regclass
       AND conname = 'workflow_refresh_drivers_validator_key_check'
  ) THEN
    ALTER TABLE public.workflow_refresh_drivers
      ADD CONSTRAINT workflow_refresh_drivers_validator_key_check
      CHECK (validator_key IN ('dispatch_source_v1', 'none'));
  END IF;
END $$;

CREATE TABLE IF NOT EXISTS public.workflow_refresh_receipts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  run_id uuid NOT NULL REFERENCES public.workflow_refresh_runs(id),
  workflow text NOT NULL CHECK (workflow IN ('dispatch','debt','ses','booking','performance')),
...
-- The only source validator currently owned by this shared foundation is the
-- Dispatch-owned source revision. Do not turn a missing/failed source read
-- into NULL, because NULL would permit an unverifiable comparison at finish.
...
BEGIN
  IF p_workflow IS DISTINCT FROM 'dispatch'
     OR p_validator_key IS DISTINCT FROM 'dispatch_source_v1'
  ...
  BEGIN
    job_id := (p_scope->>'job_id')::uuid;
  EXCEPTION WHEN invalid_text_representation THEN
    RAISE EXCEPTION 'workflow_refresh_source_unavailable';
  END;

  -- Let source lookup errors remain errors. Only a meaningful non-empty
  -- revision can be bound to a persisted receipt.
...
-- Registration is a capability declaration, not permission to select arbitrary
-- code. Until another domain contributes a validator, only Dispatch may be
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'workflow_refresh_invalid';
  END IF;

  IF p_capability = 'registered' THEN
    IF p_workflow IS DISTINCT FROM 'dispatch'
       OR p_declared_output IS DISTINCT FROM 'dispatch_refresh/v1'
       OR to_regprocedure('public.dispatch_source_version(uuid,uuid)') IS NULL
       OR to_regclass('public.dispatch_commands') IS NULL
       OR to_regclass('public.dispatch_plans') IS NULL
    THEN
...
-- Persist the driver-owned output before finish. The source read is performed
-- by this function, so a caller cannot claim that an arbitrary revision was
-- observed. Replaying the exact receipt is idempotent; a different receipt for
-- the same run is a conflict.
...
BEGIN
  ...
  -- The reference must resolve to an actual Dispatch assess command produced
  -- after this run claimed its lease. The matching plan row proves the command
  -- was persisted for this tenant/job and that its versioned result is current.
...
BEGIN
  ...
  IF p_status='completed' THEN
    ...
    -- Re-run the persisted-output validator at finish. This catches a plan or
    -- command result changed after receipt persistence, while allowing the
    -- source-specific finish check below to classify a changed source as
    -- partial. A source lookup failure aborts the finish and leaves the run
    -- running; it is never compared as NULL.
    BEGIN
