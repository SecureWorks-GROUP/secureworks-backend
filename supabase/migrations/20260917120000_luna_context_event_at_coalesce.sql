-- Luna v2 persist: source time is coalesce(event_at, occurred_at).
-- Production business_events almost never populate event_at (523 of 33761 on
-- 2026-09-17) while occurred_at is NOT NULL on every row. The nine-argument
-- persist_luna_context_revision overload rejected those sources as
-- luna_source_attribution_rejected, so the per-job pass wrote no facts.
-- Candidates already order by the same coalesce; this RPC now reads the same
-- timestamp for attribution, event_date, expiry, review_at and due-date support.
-- The five-argument overload is untouched. No business_events backfill.
-- Follow-up, not this change: populate event_at at capture and backfill the 33k
-- rows. current_job_context_facts uses the same coalesce so persisted facts stay
-- current.
...
-- Same coalesce on the luna_v2 live-source re-validation. Persist-only would
-- write facts that this view then hid. Column order is unchanged.
