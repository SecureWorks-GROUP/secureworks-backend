...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp AS $$
 WITH eligible AS (
 SELECT e.* FROM public.business_events e WHERE public.automation_lane_enabled('extraction') AND e.job_id::text=p_job_id::text
 ...
 AND EXISTS(SELECT 1 FROM public.business_events fresh WHERE fresh.job_id::text=p_job_id::text AND fresh.context_captured_at IS NOT NULL
  AND fresh.attribution_status IN ('direct','thread','single_open','single_line','luna') AND fresh.direction IS DISTINCT FROM 'outbound')
 ), anchor AS (
 SELECT e.* FROM public.business_events e WHERE e.job_id::text=p_job_id::text
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp AS $$
 ...
 JOIN public.jobs j ON j.id::text=e.job_id::text
 WHERE public.automation_lane_enabled('extraction')
 AND btrim(public.context_event_text(e))<>''
 AND EXISTS(SELECT 1 FROM public.business_events fresh WHERE fresh.job_id::text=e.job_id::text AND fresh.context_captured_at IS NOT NULL AND fresh.attribution_status IN ('direct','thread','single_open','single_line','luna') AND fresh.direction IS DISTINCT FROM 'outbound')
 AND e.attribution_status IN ('direct','thread','single_open','single_line','luna')
 AND NOT EXISTS(SELECT 1 FROM public.context_extraction_event_receipts r WHERE r.event_id=e.id AND r.job_id=j.id AND r.extractor_version='luna_v2')
 AND NOT EXISTS(SELECT 1 FROM public.context_extraction_runs r WHERE r.job_id=j.id AND r.run_date=(now() AT TIME ZONE 'Australia/Perth')::date AND r.phase='extraction' AND r.status IN ('done','skipped'))
 GROUP BY j.id ORDER BY EXISTS(SELECT 1 FROM public.context_extraction_runs retry WHERE retry.job_id=j.id AND retry.run_date=(now() AT TIME ZONE 'Australia/Perth')::date AND retry.phase='extraction' AND retry.status IN ('running','failed')) DESC,min(coalesce(e.event_at,e.occurred_at)),j.id LIMIT greatest(0,least(coalesce(p_limit,400),400))
$$;
