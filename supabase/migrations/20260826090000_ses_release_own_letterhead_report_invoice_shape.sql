-- SES release commit: accept own-letterhead roof report+invoice (no photo).
--
-- Live Gwelup SWMS-26980 (own_template_roof) drafts REPORT + INVOICE because
-- the family matrix stamps photo_route=not_applicable and still owes a real
-- report email. commit_ses_release_revision_v1 accepted only:
--   3-route universal (report/photo/invoice)
--   2-route AJS (report_invoice/photo)
--   1-route invoice (portal roof)
-- so SEND IT failed at prepare: "release routes must be ordered
-- report/photo/invoice (universal) or report_invoice/photo (AJS)".
--
-- WIDEN-ONLY. One extra ELSIF, one extra flag on the refusal gate.
-- Error message unchanged (still fires only for refused shapes).
...
DECLARE
  ...
  own_letterhead_two boolean := false;
BEGIN
  ...
  ELSIF route_count = 2
     ...
     AND (p_routes->0->>'ordinal')::integer = 0
     AND (p_routes->1->>'ordinal')::integer = 1 THEN
    own_letterhead_two := true;
  END IF;

  IF NOT universal AND NOT ajs_two AND NOT report_only_one
     AND NOT own_letterhead_two THEN
    RAISE EXCEPTION
