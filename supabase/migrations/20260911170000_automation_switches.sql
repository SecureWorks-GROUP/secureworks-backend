...
--
-- An unexpected command aborts the migration rather than leaving an ungated
-- scheduled caller. No command text is printed (it may contain credentials).
--
...
BEGIN
  ...
  FOR r IN SELECT * FROM public.automation_switch_cron_lanes() LOOP
    ...
    LOOP
      ...
      IF right(v_cmd, length(v_sfx)) = v_sfx THEN
        cron_jobname := r.cron_jobname; cron_jobid := j.jobid;
        outcome := 'already_wrapped'; RETURN NEXT; CONTINUE;
      END IF;

      IF v_cmd !~* '^select\s' OR position(';' in v_cmd) > 0 OR v_cmd ~* '\swhere\s' OR v_cmd ILIKE '%automation_lane_enabled%' THEN
        cron_jobname := r.cron_jobname; cron_jobid := j.jobid;
        RAISE EXCEPTION 'Cannot safely gate automation cron job % (id %)', r.cron_jobname, j.jobid;
      END IF;
