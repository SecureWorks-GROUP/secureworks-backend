-- Down for 20260923230000: restore the live production ladder body byte for byte
-- (md5(prosrc) e6d5a8ec5ab942dff7f288d579d1b8f4, read from production 23 Sep 2026): step 1 matches any
-- xero_invoices.invoice_number and any holding job again. No rows are moved either
-- way; rows placed under L1 keep their placement.
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '60s';

-- Refuse to overwrite a later ladder change: only the L1 body (or the restored
-- live body, for a repeated rollback) may be replaced.
DO $guard$
...
BEGIN
 ...
 IF live_md5 IS NULL OR live_md5 NOT IN ('acb80ebe792beeb7e5b537643bf9f184','e6d5a8ec5ab942dff7f288d579d1b8f4') THEN
  RAISE EXCEPTION 'context_ladder_rollback_mismatch: live resolve_context_attribution md5 % is not the L1 body; a later ladder change must be rolled back first', coalesce(live_md5,'<missing>');
 END IF;
END $guard$;
