...
calendar_job_family_migration_sha() {
...
sales_booking_thread_facts_migration_sha() {
  shasum -a 256 "$SALES_BOOKING_THREAD_FACTS_MIGRATION" | awk '{print $1}'
}

sales_booking_roster_migration_sha() {
  shasum -a 256 "$SALES_BOOKING_ROSTER_MIGRATION" | awk '{print $1}'
}
...
calendar_job_family_row = {
    "function_name": "ops-api",
    "migration_version": "20260916000000",
    "expected_migration_name": "calendar_events_job_family",
    "expected_statement_sha256": os.environ["CALENDAR_JOB_FAMILY_EXPECTED_SHA"],
    "actual_migration_version": "20260916000000",
    "actual_migration_name": "calendar_events_job_family",
    "actual_statement_count": 1,
...
sales_booking_packs_row = {
    ...
    "expected_migration_name": "sales_booking_packs",
    "expected_statement_sha256": os.environ["SALES_BOOKING_PACKS_EXPECTED_SHA"],
    "actual_migration_version": "20260917130000",
    "actual_migration_name": "sales_booking_packs",
    "actual_statement_count": 1,
    "actual_statement_sha256": os.environ["SALES_BOOKING_PACKS_EXPECTED_SHA"],
    "missing_markers": [],
}
sales_booking_thread_facts_row = {
    "function_name": "ops-api",
    "migration_version": "20260917180000",
    "expected_migration_name": "sales_booking_thread_facts",
    "expected_statement_sha256": os.environ["SALES_BOOKING_THREAD_FACTS_EXPECTED_SHA"],
    "actual_migration_version": "20260917180000",
    "actual_migration_name": "sales_booking_thread_facts",
    "actual_statement_count": 3,
    "actual_statement_sha256": None,
    "missing_markers": [],
}
sales_booking_roster_row = {
    "function_name": "ops-api",
    "migration_version": "20260917200000",
    "expected_migration_name": "sales_booking_roster",
    "expected_statement_sha256": os.environ["SALES_BOOKING_ROSTER_EXPECTED_SHA"],
    "actual_migration_version": "20260917200000",
    "actual_migration_name": "sales_booking_roster",
    "actual_statement_count": 3,
    "actual_statement_sha256": None,
    "missing_markers": [],
}
...
main() {
  ...
  echo
  if [[ ! -f "$PREFLIGHT" || ! -f "$MANIFEST" || ! -f "$MIGRATION" || ! -f "$MEDIA_MIGRATION" || ! -f "$FRESH_HEALTH_MIGRATION" || ! -f "$U5_U6_MIGRATION" || ! -f "$FENCE_HARDENING_MIGRATION" || ! -f "$DOCS_READY_MIGRATION" || ! -f "$SIBLING_EVIDENCE_MIGRATION" || ! -f "$PORTAL_CAPTURE_MIGRATION" || ! -f "$SEED_SCOPE_MIGRATION" || ! -f "$HUGO_NOTIFICATION_MIGRATION" || ! -f "$CYCLE_UNIQUENESS_MIGRATION" || ! -f "$PDF_EXTRACTION_MIGRATION" || ! -f "$INTAKE_SETTLEMENT_MIGRATION" || ! -f "$BOARD_V2_PREVIEW_MIGRATION" || ! -f "$VAULT_SYNC_MIGRATION" || ! -f "$SES_RECOVERY_MIGRATION" || ! -f "$ROOF_INITIAL_CYCLE_MIGRATION" || ! -f "$INVOICE_BOUND_ADOPT_MIGRATION" || ! -f "$RELEASE_ROUTE_KIND_MIGRATION" || ! -f "$MAILER_OPS_SEND_MIGRATION" || ! -f "$ECHO_CODE_APPROVAL_MIGRATION" || ! -f "$DEBT_PICTURE_MIGRATION" || ! -f "$CALENDAR_JOB_FAMILY_MIGRATION" || ! -f "$SALES_BOOKING_PACKS_MIGRATION" || ! -f "$SALES_BOOKING_THREAD_FACTS_MIGRATION" || ! -f "$SALES_BOOKING_ROSTER_MIGRATION" ]]; then
