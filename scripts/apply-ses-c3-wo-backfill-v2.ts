...
/**
 * SES Phase C3 tranche 2 — re-attach the reference/subject-matched builder
 * work-order artifacts for the closed, hand-adjudicated tier-B/C card set.
 *
 * Scope is EXACTLY the fixture in `scripts/ses-c3-wo-backfill-v2.fixture.txt`:
 * the board cards whose missing work-order PDF still has a servable artifact in
 * the private `makesafe-emails` bucket that a human adjudicated card by card.
 * The per-card agreement matrix, the discriminators used where a claim carried
 * more than one work order, and every SKIP verdict with its reason live in
 * `scripts/ses-c3-wo-backfill-v2.adjudication.json`.
 *
 * The fixture is closed and hand-adjudicated. This script has no discovery step
 * by design: no card a human did not adjudicate can be written by it.
 *
 * HOW THIS DIFFERS FROM TRANCHE 1 (`apply-ses-c3-wo-backfill-v1.ts`). Tranche 1
 * could bind every artifact to the card's OWN `makesafe_intake_cases` →
 * `makesafe_intake_case_sources.attachment_refs` chain and re-prove identity
 * server-side from the case's `builder_wo_canonical`. These artifacts are NOT
 * case-bound — that is what made them tier B/C — so the runtime identity guard
 * is a different pair, both recomputed from production on every read:
 *
 *   1. the card's own builder reference digits are still present in the
 *      artifact's `pdf_extraction_text` as a whole number token (digit
 *      boundaries, not a substring — see `loadCardStates`), and
 *   2. the md5 of that extracted text still equals the value the adjudication
 *      was made against.
 *
 * (2) is the load-bearing half. The adjudication rested on the document's
 * CONTENT, so pinning the exact adjudicated text means a re-extraction, a
 * replacement or any drift refuses the write instead of silently attaching a
 * document nobody adjudicated. The artifact `sha256` and `name` are pinned too.
 *
 * Transport split, by design (identical to tranche 1):
...
export const RUN_LABEL = "ses-c3-wo-backfill-tranche2-v1";
export const PROVENANCE_NOTE =
  "SES Phase C3 tranche 2 work-order re-attach; per-card adjudication 2026-08-01, captain phase-C plan";
export const DEFAULT_FIXTURE_PATH = "scripts/ses-c3-wo-backfill-v2.fixture.txt";
export const ADJUDICATION_PATH =
  "scripts/ses-c3-wo-backfill-v2.adjudication.json";

/**
 * The board-wide C1 measurement entrypoint.
 *
 * `scripts/ses-c2-measure-board-evidence.ts` batches the SHIPPED C1 ruler —
 * `buildSesCardEvidenceInventory` from `scripts/ses-measure-card-evidence.ts`
 * over `supabase/functions/ops-api/makesafe_evidence_requirements.ts` — across
 * every board card, reading only through the Management API. It is used here
 * rather than the single-card entrypoint for two reasons: it is the same ruler
 * (so this script still cannot measure with a private re-implementation), and a
 * whole-board sweep lets `verify` prove that NO board card moved stage, not
 * just the fixture cards.
 */
const MEASURE_ENTRYPOINT = "scripts/ses-c2-measure-board-evidence.ts";
...
export interface CardState {
  ...
  identity_reference_matches: boolean;
  identity_text_md5: string | null;
...
/** The digits the runtime identity guard looks for in the extracted PDF text. */
export function referenceDigits(reference: string): string {
...
export function parseFixture(text: string): FixtureRow[] {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (
      !card || !jobId || !reference || !attachmentId || !sha256 ||
      !pdfTextMd5 || !fileName
    ) {
    ...
    if (!/^[0-9a-f]{32}$/i.test(pdfTextMd5)) {
      throw new Error(`fixture line ${index + 1} has a malformed pdf_text_md5`);
    }
    // A reference with no digits would make the identity guard vacuous: the
    // SQL predicate would search for the empty string and always match.
    if (!referenceDigits(reference)) {
      throw new Error(
        `fixture line ${index + 1} has a reference with no digits`,
      );
    }
 ...
 *
 * A card is written only when every leg of the adjudication still holds AND the
 * card is still in the exact state the dry run recorded. Anything else is
...
export function evaluateEligibility(
  ...
  if (!state.identity_reference_matches) {
    return refuse("identity_reference_mismatch");
  }
  // The adjudication was made against this exact extracted text. Any drift and
  // the document on file is no longer the one a human read.
  if (
    (state.identity_text_md5 || "").toLowerCase() !==
      row.pdfTextMd5.toLowerCase()
  ) {
    return refuse("adjudicated_pdf_text_drift");
  }
...
async function managementQuery(
  ...
  const response = await fetch(MANAGEMENT_QUERY_URL, {
    ...
    headers: {
      ...
      "User-Agent": "SecureWorks-SES-C3-WO-Backfill/2.0",
    },
    body: JSON.stringify({
      query,
 ...
 *
 * Every leg the adjudication rests on is recomputed here from production rather
 * than trusted from the adjudication file: the artifact must still carry the
 * card's own reference digits in its extracted PDF text, that text must still
 * hash to the adjudicated value, and its storage object must still resolve in
 * the private bucket.
 */
async function loadCardStates(
  ...
  const pairs = rows.map((row) =>
    ...
    }::uuid,${sqlText(referenceDigits(row.reference))})`
  ).join(",");
  const query = `
with fixture(card, job_id, attachment_id, reference_digits) as (values ${pairs})
select f.card,
       ...
       (
         ...
         and f.reference_digits <> ''
         -- Digit boundaries, not a substring. The four-digit builder references
         -- (BWCWA6771, BWCWA6773) are substrings of unrelated five- and
         -- six-digit job and purchase-order numbers, and a plain
         -- position()/like test matches those too. The adjudication rejected
         -- exactly those candidates, so the runtime guard has to be able to as
         -- well. reference_digits is digits-only by construction.
         and ea.pdf_extraction_text ~ ('(^|[^0-9])' || f.reference_digits || '([^0-9]|$)')
       ) as identity_reference_matches,
       md5(ea.pdf_extraction_text) as identity_text_md5,
...
order by f.card;`;
  ...
  for (const raw of data) {
    states.set(String(raw.card), {
      ...
      identity_reference_matches: raw.identity_reference_matches === true,
      identity_text_md5: raw.identity_text_md5 ?? null,
...
async function stampProvenance(
  ...
  const provenance = {
    ...
    source: `${ADJUDICATION_PATH} (per-card adjudication, tier B/C)`,
    source_bucket: SOURCE_BUCKET,
    source_email_attachment_id: row.attachmentId,
    source_sha256: row.sha256,
    adjudicated_pdf_text_md5: row.pdfTextMd5,
    builder_reference: row.reference,
  };
  const query = `
with stamped as (
...
;`,
  ...
  const live = data.length === 1
    ...
    : null;
  return authorizeResumeStamp(entry, row, live);
}
...
export interface BoardStageEntry {
  stage: string | null;
  computed_stage: string | null;
  display_overlay_stage: string | null;
}

/** Reduce one raw C2 board row to the fields this backfill is accountable for. */
export function summariseMeasuredCard(raw: any): MeasureSummary {
  const item = raw?.inventory?.builder_wo_doc || {};
  return {
    card: String(raw?.job_ref ?? ""),
    stage: raw?.stage ?? null,
    computed_stage: raw?.computed_stage ?? null,
    display_overlay_stage: raw?.display_overlay_stage ?? null,
    verdict: raw?.reading?.verdict ?? null,
    builder_wo_doc_present: item?.present === true,
    builder_wo_doc_detail: item?.detail ?? null,
  };
}

/** The stage triple of every board card, used to prove nothing else moved. */
export function boardStageIndex(
  rawCards: any[],
): Record<string, BoardStageEntry> {
  const index: Record<string, BoardStageEntry> = {};
  for (const raw of rawCards) {
    index[String(raw?.job_ref ?? "")] = {
      stage: raw?.stage ?? null,
      computed_stage: raw?.computed_stage ?? null,
      display_overlay_stage: raw?.display_overlay_stage ?? null,
    };
...
export function boardStageDrift(
  ...
  for (const [card, was] of Object.entries(before)) {
    ...
    if (
      ...
      was.display_overlay_stage !== now.display_overlay_stage
    ) {
      drifted.push(card);
    }
  }
...
/**
 * Run the board-wide C1 measurement once and keep both the fixture-card detail
 * and the whole-board stage index. The entrypoint is invoked as a subprocess
 * deliberately: it is the shipped measurement contract, so this script cannot
 * accidentally measure with a private re-implementation.
 */
async function measureBoard(
  rows: readonly FixtureRow[],
): Promise<
  { cards: MeasureSummary[]; board: Record<string, BoardStageEntry> }
> {
  const outPath = await Deno.makeTempFile({
    prefix: "ses-c3-t2-measure-",
    suffix: ".json",
  });
  try {
    ...
    if (!result.success) {
      ...
      throw new Error(`C1 board measure failed: ${stderr.trim()}`);
    }
    const parsed = JSON.parse(await Deno.readTextFile(outPath));
    const rawCards: any[] = parsed?.cards || [];
    const byRef = new Map<string, any>(
      rawCards.map((raw) => [String(raw?.job_ref ?? ""), raw]),
    );
    const cards: MeasureSummary[] = [];
    for (const row of rows) {
      const raw = byRef.get(row.card);
      if (!raw) throw new Error(`${row.card} is not on the measured board`);
      cards.push(summariseMeasuredCard(raw));
    }
    return { cards, board: boardStageIndex(rawCards) };
  } finally {
...
function printHelp(): void {
  console.log(
    `description: Re-attach the hand-adjudicated tier-B/C builder work-order PDFs (SES Phase C3 tranche 2)
...
environment[2]{name,required_for}:
  ...
  SW_API_KEY,apply
examples[4]:
  "scripts/apply-ses-c3-wo-backfill-v2.ts --mode measure --output <c1-before.json>"
  "scripts/apply-ses-c3-wo-backfill-v2.ts --mode dry-run --output <dry-run.json>"
  "scripts/apply-ses-c3-wo-backfill-v2.ts --mode apply --baseline <dry-run.json> --ledger <ledger.json>"
  "scripts/apply-ses-c3-wo-backfill-v2.ts --mode verify --baseline <dry-run.json> --ledger <ledger.json> --before <c1-before.json> --after <c1-after.json> --output <verify.json>"`,
  );
}
...
async function runMeasure(
  ...
  const { cards, board } = await measureBoard(rows);
  await writeEvidence(outputPath, {
    run_label: RUN_LABEL,
    mode: "measure",
    contract:
      `${MEASURE_ENTRYPOINT} (batches the C1 ruler scripts/ses-measure-card-evidence.ts)`,
    cards,
    board_cards: Object.keys(board).length,
    board_stage_index: board,
  }, overwrite);
  summarise("measure", {
    cards: cards.length,
    builder_wo_doc_present: cards.filter((m) =>
      m.builder_wo_doc_present
    ).length,
    board_cards: Object.keys(board).length,
  });
}
...
async function runVerify(
  ...
  for (const row of rows) {
    ...
    if (planned?.before) {
      ...
      if (planBefore.identity_text_md5 !== state.identity_text_md5) {
        problems.push("source_pdf_text_changed");
      }
  ...
  // Whole-board assertion: no card the tranche did not touch may have moved.
  const boardDrift = boardStageDrift(
    beforeFile.board_stage_index || {},
    afterFile.board_stage_index || {},
  );
  if (boardDrift.length) failures++;
  ...
  await writeEvidence(outputPath, {
    ...
    cards: results,
    board_cards_compared:
      Object.keys(beforeFile.board_stage_index || {}).length,
    board_stage_drift: boardDrift,
    ok: failures === 0,
    failures,
  }, overwrite);
  summarise("verify", {
    ...
    board_stage_drift: boardDrift.length,
    failures,
  });
