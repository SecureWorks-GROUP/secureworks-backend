...
const GIRTH_BANDS: [number, number][] = [[0, 100], [101, 150], [151, 200], [
  201,
  300,
], [301, 400]];

const ENGINE_ALLOWANCES: Record<
  string,
  { key: string; basis: AllowanceRow["basis"]; description: string }
> = {
  ...
  "fixings": {
    key: "fixings",
    basis: "per_m2",
    description: "Fixings per m2 of roof",
  },
};
...
export function buildPlan(observations: Observation[]): ImportPlan {
  ...
  const addItem = (item: CanonicalItem) => {
    ...
    if (!prior || (!prior.reconciled && item.reconciled)) {
      items.set(item.item_key, item);
    }
  };
  const addCut = (
    itemKey: string,
  ...
  for (const o of observations) {
    ...
    const allowanceKey = o.store === "s07_patio_engine_snapshot"
      ...
      : o.store === "s05_patio_hardcoded" &&
          o.source_key === "DEFAULT_RATES.Fixings ($/sqm)"
      ? "fixings"
      : undefined;
    if (allowanceKey && ENGINE_ALLOWANCES[allowanceKey]) {
      const a = ENGINE_ALLOWANCES[allowanceKey];
      if (!o.value) {
        plan.zeroSentinels.push({
          item_key: `allowance:${a.key}`,
          store: o.store,
    ...
    if (o.kind === "stock_lengths") {
      if (o.stock_lengths_mm?.length) {
        ...
        const rule = r.item.category === "steel"
          ? steelCutRule(r.item.item_key)
          : "nest";
        addCut(
          r.item.item_key,
          rule,
        ...
        addCut(
          ...
          "cut_to_size",
          o,
          "the supplier cuts to length (patio STOCK_LENGTH_WASTE_CONFIG)",
        );
      }
      continue;
    }
    ...
    const notes = [
      ...
      o.note,
      o.claimed_blessed
        ? `source comment claims blessed ${o.claimed_blessed}; loaded provisional`
        : null,
  ...
  const flashing = [...costs.values()].filter((c) =>
    c.item_key.startsWith("flashing-girth-")
  );
  ...
  for (const [lo, hi] of GIRTH_BANDS) {
    ...
    const refs = inBand.map((c) =>
      `${c.item_key} ${c.supplier} $${c.cost_ex_gst.toFixed(2)}`
    ).join("; ");
    allowances.push({
      family: "patio",
      ...
      evidence_ref:
        `mean of ${inBand.length} newest invoice/order rates in band`,
      evidence_note: refs,
      fingerprint: fingerprint([
        "allowance",
        "flashing-band",
  ...
  const familyAverage = observations.find((o) =>
    o.store === "s05_patio_hardcoded" &&
    o.source_key === "DEFAULT_RATES.Ridge Cap" && (o.value ?? 0) > 0
  );
  if (familyAverage?.as_at && familyAverage.value) {
    allowances.push({
      ...
      description:
        "Flashing per metre when the girth is not known (family average)",
      basis: "per_lm",
      girth_min_mm: null,
      girth_max_mm: null,
      ...
      evidence_note:
        "family average of 13 invoice lines across 3 suppliers (patio PR 102, 13 Jun)",
      fingerprint: fingerprint([
        "allowance",
  ...
  for (const row of [...costs.values(), ...stock.values(), ...cuts.values()]) {
    if (!items.has(row.item_key)) {
      throw new Error(`plan: row for unknown item ${row.item_key}`);
    }
  }

  plan.items = [...items.values()].sort((a, b) =>
    a.item_key.localeCompare(b.item_key)
  );
  plan.costs = [...costs.values()];
  plan.stock = [...stock.values()];
  plan.cuts = [...cuts.values()];
  plan.diff = [...diff.values()].sort((a, b) =>
    a.item_key.localeCompare(b.item_key)
  );
  return plan;
}
    ...
    out.push(
      `INSERT INTO public.price_book_items (item_key, family, category, description, unit, note, created_by, import_fingerprint) VALUES (${
        [
          ...
          i.reconciled
            ? null
            : "imported as-is; not reconciled with other sources",
          recordedBy,
          `${IMPORT_VERSION}|item|${i.item_key}`,
        ].map(lit).join(", ")
  ...
  for (const s of plan.stock) {
    out.push(
      `INSERT INTO public.price_book_stock_lengths (item_key, supplier, lengths_mm, as_at, evidence_kind, evidence_ref, recorded_by, import_fingerprint) VALUES (${
        ...
        [s.as_at, "tool_constant", s.evidence_ref, recordedBy, s.fingerprint]
          .map(lit).join(", ")
      }) ON CONFLICT (import_fingerprint) DO NOTHING;`,
    );
  }
...
const LIVE_TOOL_STORES = new Set<StoreId>([
  "s01_fence_cost_prices",
  "s05_patio_hardcoded",
]);
...
export function compare(entry: DiffEntry): ItemComparison {
  ...
  const evidence = priced.filter((o) =>
    o.store === "s10_wiki_supplier_csv" ||
    o.store === "s09_material_price_ledger"
  );
  const latestEvidence =
    [...evidence].sort((a, b) =>
      (b.as_at ?? "").localeCompare(a.as_at ?? "")
    )[0] ?? null;
  ...
  const deltaPct = latestEvidence && toolLatest
    ? Math.round(
      ((toolLatest.value! - latestEvidence.value!) / latestEvidence.value!) *
        1000,
    ) / 10
    : null;
...
export function summarise(plan: ImportPlan): DiffSummary {
  ...
  return {
    ...
    reconciled_multi_source:
      plan.diff.filter((d) =>
        d.reconciled && new Set(d.observations.map((o) => o.store)).size > 1
      ).length,
    tool_vs_evidence_compared:
      comparisons.filter((c) => c.deltaPct != null).length,
    tool_vs_evidence_over_5pct:
      comparisons.filter((c) => c.deltaPct != null && Math.abs(c.deltaPct) > 5)
        .length,
    tool_only:
      comparisons.filter((c) => c.toolLatest && !c.latestEvidence).length,
    evidence_only:
      comparisons.filter((c) => c.latestEvidence && !c.tool.length).length,
    unpriced_items:
      plan.items.filter((i) => !pricedKeys.has(i.item_key)).length,
    ...
    claimed_blessed_not_trusted:
      plan.diff.flatMap((d) => d.observations).filter((o) => o.claimed_blessed)
        .length,
  };
}
...
export function renderDiff(
  ...
  lines.push(
    `Generated ${meta.generatedAt} by \`scripts/quote-v2/price_book_import.ts\` (${IMPORT_VERSION}), dry run.`,
  );
  lines.push(
    `Sources read at: ${
      Object.entries(meta.refs).map(([k, v]) => `${k} \`${v}\``).join(", ")
    }.`,
    "",
  );
  lines.push(
  ...
  const label: Record<keyof DiffSummary, string> = {
    ...
    tool_vs_evidence_compared:
      "Items where a tool constant can be compared with invoice or supplier evidence",
    tool_vs_evidence_over_5pct:
      "...of those, tool differs from latest evidence by more than 5%",
    ...
    excluded_rows:
      "Rows excluded (dead store, compound line, one-off, not a price)",
    ...
    claimed_blessed_not_trusted:
      "Values a tool comment calls blessed (loaded provisional)",
  };
  for (const [k, v] of Object.entries(s)) {
    lines.push(`| ${label[k as keyof DiffSummary]} | ${v} |`);
  }
  lines.push("");
  ...
  for (const c of withBoth) {
    const toolCell = c.tool.map((o) =>
      `${STORE_LABEL[o.store].split(" ")[0]}: ${money(o.value)}`
    ).join("<br>");
    const csv = c.evidence.filter((o) => o.store === "s10_wiki_supplier_csv");
    const csvCell = csv.length
      ? `${csv.length} rows, ${
        money(Math.min(...csv.map((o) => o.value!)))
      } to ${money(Math.max(...csv.map((o) => o.value!)))}`
      : "none";
    const le = c.latestEvidence!;
    lines.push(
      `| \`${c.entry.item_key}\` | ${c.entry.unit} | ${toolCell} | ${csvCell} | ${
        money(le.value)
      } ${le.as_at} ${le.supplier}${
        le.conversion ? ` (${le.conversion})` : ""
  ...
  for (
    const c of comparisons.filter((c) => c.toolLatest && !c.latestEvidence)
  ) {
    const blessed = c.tool.find((o) => o.claimed_blessed)?.claimed_blessed;
    const vals = [
      ...new Set(
        c.tool.filter((o) => o.value != null).map((o) =>
          `${STORE_LABEL[o.store].split(" ")[0]}: ${money(o.value)}`
        ),
      ),
    ].join("<br>");
    const spread = new Set(
        c.tool.filter((o) => o.value != null).map((o) => o.value),
      ).size >
        1
      ? "tools disagree"
      : "";
    lines.push(
      `| \`${c.entry.item_key}\` | ${c.entry.unit} | ${vals} | ${
        [spread, blessed ? `comment says blessed ${blessed}` : ""].filter(
          Boolean,
        ).join("; ")
      } |`,
    );
  ...
  for (const c of comparisons) {
    ...
    const cands = comparisons.filter((x) =>
      re.test(x.entry.item_key) && x.latestEvidence
    )
      .map((x) =>
        `W${x.entry.item_key.match(re)![1]}: ${money(x.latestEvidence!.value)}`
      );
    lines.push(
      `| \`${c.entry.item_key}\` | ${
        c.tool.filter((o) => o.value != null).map((o) => money(o.value)).join(
          ", ",
        )
      } | ${cands.join(", ") || "none on file"} |`,
    );
  }
  lines.push("");
  ...
  for (
    const c of comparisons.filter((c) => c.latestEvidence && !c.tool.length)
  ) {
    const le = c.latestEvidence!;
    lines.push(
      `| \`${c.entry.item_key}\` | ${c.entry.unit} | ${
        money(le.value)
      } ${le.as_at} ${le.supplier}${
        le.staged ? " (staged for blessing)" : ""
      } | ${c.evidence.length} |`,
    );
  }
  lines.push("");
  ...
  for (const o of plan.legacySell) {
    lines.push(
      `| ${STORE_LABEL[o.store]} | ${o.source_key} | ${
        money(o.value)
      } | ${o.source_unit} |`,
    );
  }
  ...
  for (const o of plan.markups) {
    lines.push(`| ${STORE_LABEL[o.store]} | ${o.source_key} | ${o.value} |`);
  }
  lines.push(
    "",
  ...
  for (const a of plan.allowances) {
    lines.push(
      `| ${a.allowance_key} | ${a.basis} | ${
        a.girth_min_mm == null
          ? ""
          : `${a.girth_min_mm} to ${a.girth_max_mm} mm`
      } | $${a.cost_ex_gst.toFixed(2)} | ${
        a.evidence_note ?? a.evidence_ref
      } |`,
    );
  }
  lines.push("");
  ...
  if (plan.unresolved.length) {
    ...
    for (const u of plan.unresolved) {
      lines.push(
        `- ${
          STORE_LABEL[u.observation.store]
        } \`${u.observation.source_key}\`: ${u.reason}`,
      );
    }
    lines.push("");
  }
