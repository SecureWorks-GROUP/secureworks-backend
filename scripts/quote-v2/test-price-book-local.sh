...
if [[ ! "$ADMIN_URL" =~ ^postgres(ql)?://([^/?#@]+@)?(127\.0\.0\.1|localhost)(:[0-9]+)?/[^/?#]+$ ]]; then
  echo "error: PRICE_BOOK_LOCAL_DATABASE_URL must target localhost" >&2
  exit 2
fi
...
psql_local "$DB_URL" -X -q -v ON_ERROR_STOP=1 \
  ...
  -f "$ROOT/supabase/migrations/20260925010000_quote_price_book.sql" 2>&1 | sed '/SET LOCAL/d'

count() {
  psql_local "$DB_URL" -X -tA -c "select (select count(*) from price_book_items) || '/' || (select count(*) from price_book_costs) || '/' || (select count(*) from price_book_stock_lengths) || '/' || (select count(*) from price_book_cut_rules) || '/' || (select count(*) from price_book_allowances)"
}

run_import() {
  (cd "$ROOT" && deno run --allow-read --allow-write --allow-env=HOME,PATH --allow-run=git,psql \
...
zero=$(psql_local "$DB_URL" -X -tA -c "select count(*) from price_book_costs where cost_ex_gst <= 0")
blessed=$(psql_local "$DB_URL" -X -tA -c "select count(*) from price_book_costs where not provisional")
[ "$zero" = "0" ] || { echo "error: a zero cost was loaded" >&2; exit 1; }
[ "$blessed" = "0" ] || { echo "error: the import blessed a row" >&2; exit 1; }

psql_local "$DB_URL" -X -c "select status, count(*) from price_book_current_costs() group by 1 order by 1"
psql_local "$DB_URL" -X -c "select item_key, status, cost_ex_gst, per_length_mm, supplier, as_at, evidence_kind, stock_lengths_mm, cut_rule from price_book_current_costs(array['steel-rhs-100x50x2','stratco-qs-slat-65-col-lm','fence-panel-kit-h1800-w2380-post2400','fence-delivery-rr','concrete-kwikset-20kg'])"
psql_local "$DB_URL" -X -c "select family, value, status from price_book_current_markup('patio') union all select family, value, status from price_book_current_markup('fencing')"
