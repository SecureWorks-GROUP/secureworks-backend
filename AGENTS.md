...
`single_line`, `luna`), once per job, and stamps moved rows
`capture_mode='relink'`; never add a second reopen path, and a new reason
(sites' `party_linked`) replaces its body.
