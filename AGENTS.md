...
New code that saves evidence into `business_events` calls the SQL writer
`capture_business_event(p_row jsonb)` (`20260924100000`), never a direct insert
or `recordEvidence`. It is keyed on `provider_message_id` only, stamps
`occurred_at` itself, refuses writer-owned and unknown fields, requires
`metadata.capture_mode` (`live`, `backfill`, `relink`), lets the ladder place
the row, and on a duplicate may only upgrade a non-direct link to a verified
direct writer job id (`metadata.upgraded_from`). A GHL message row is built only
by `_shared/evidence/ghl_message.ts` (`thread_key` always null). Its PostgreSQL
contract feeds rows generated by that builder; `ghl_message_contract_rows_test.ts`
fails if they drift.
...
number drifts every release). Operator-facing `pre_xero_docs_ready` and
`review_state` follow the same honesty gate as the board projection
...
## Trade Invoices Persist Gross, Super, Net Pay, And The GST Choice
...
SG rate, computes gross earned / super remittance / worker withhold / net pay /
optional GST, and builds Xero labour lines plus one labelled worker-withhold
minus line. Captain 2026-09-18: `super_amount` stays 12% of gross (bookkeeper /
fund); cash withheld from the trade is 6%; the company contributes the other
6%. On $1,000: super $120, worker $60, company $60, `net_pay` $940, company
total out $1,060. `total_inc` stays `gross + GST` (supply face, not company
cost). Historical 12% carve-out rows remain valid and are not rewritten. Once
Xero creates a bill, its ID is checkpointed locally before returned-line
reconciliation; only a reconciled split may advance to `pushed_to_xero`. Every
create/retry uses the persisted invoice ID as its Xero idempotency key, and any
external Xero identity blocks reject/delete. Draft replacement uses
...
must fail closed on Xero retry rather than inventing history. Apply
...
switch (`SALES_BOOKING_BOOK_EXECUTE`) turns booking and that Outlook copy
...
`monitored_mailboxes` (the T7 draft table, built on by EM1, `20260924213000`)
is the new Outlook poller's source list, behind flag `email_capture_v2` (off
until EM-M2). The old monitor-inbox path polls only
`monitor-inbox/legacy_mailboxes.ts` and must never read that table: EM1 drops
its `last_polled_at`, so the pre-EM1 query is refused (contract row E22). Change a
