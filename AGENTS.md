...
written only by `context_catchup_request`): a listed job with unread evidence is
due once inside that same judgement, and its first `done` extraction run closes
it; field contract in `docs/context/pipeline-status.md`. The one unread definition is
