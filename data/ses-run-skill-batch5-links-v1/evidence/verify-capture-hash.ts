...
for (const r of rows) {
  // deno-lint-ignore no-explicit-any -- fields are selected dynamically from evidence JSON.
  const content: any = {};
  ...
  const spellings = [
    r.captured_at,
    String(r.captured_at).replace(" ", "T").replace("+00", "+00:00"),
  ];
  for (const s of spellings) {
    const digest = await sesPortalCaptureRevisionHash({
      ...content,
      captured_at: s,
    });
    console.log(
      `${r.job_number}  spelling="${s}"  match=${
        digest === r.makesafe_content_hash
      }`,
    );
  }
}
