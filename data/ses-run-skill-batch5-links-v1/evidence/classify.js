() => {
  ...
  // ---- signals -------------------------------------------------------------
  const LOCK_RE = /locked and is no longer available for editing or submission|this form (?:has been|is) locked|no longer available for editing/i;
  ...
  const EXPIRED_RE = /this link is no longer active or has expired|link (?:has )?expired|no longer active|link is not valid|invalid link|link has been revoked/i;
  ...
  const nOfN = (text.match(/\b(\d{1,3})\s+of\s+(\d{1,3})\b/) || [null, null, null]);
  ...
  const jobNo = (text.match(/Job Number\s*\n\s*([A-Z0-9][A-Z0-9\-\/]{2,30})/i) || [null, null])[1];
  ...
  const formPresent = (hasSubmit || hasSave || autosave || total !== null) && html.length > 20000;
  ...
  if (locked) {
    ...
    why = "lock banner present" + (answered !== null ? ` (${answered} of ${total} answered)` : "");
  } else if (formPresent) {
    state = "present-but-not-submitted";
    why = `editable form rendered (submit=${hasSubmit}, save=${hasSave}, autosave=${autosave}` +
    ...
    state = "indeterminate";
    why = `pre-form consent shell, form not yet mounted (bodyHtml=${html.length}b, innerText=${text.length}c)`;
  } else if (
    ...
    why = "Prime generic /not-found page — not evidence of expiry; re-enter via the stored /share/ URL";
  } else {
    state = "indeterminate";
    why = `unrecognised page shape (bodyHtml=${html.length}b, innerText=${text.length}c)`;
  }
  ...
  document.querySelectorAll("div.prime-flex").forEach((row) => {
    ...
    if (!/^(Customer|Site Address|Client|Address|Contact|Phone|Email)$/i.test(label)) return;
  ...
  if (boxes.length) {
    ...
    boxes.forEach(([l, t, w, h]) => {
      ...
      d.style.cssText = `position:absolute;left:${l}px;top:${t}px;width:${w}px;height:${h}px;` +
  ...
  return {
    url: location.href, state, why, locked, formPresent, availableUntil,
    answered, total, jobNo, formTitle, hasSubmit, hasSave, autosave,
    bodyHtmlLen: html.length, innerTextLen: text.length,
