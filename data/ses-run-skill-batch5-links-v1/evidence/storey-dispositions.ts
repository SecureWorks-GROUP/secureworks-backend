...
const out: any[] = [];
for (const r of rows) {
  const job = { id: r.job_number, job_number: r.job_number, status: r.status, notes: r.notes,
                metadata: r.metadata, scope_json: r.scope_json, site_suburb: null };
  const row: any = buildRoofStoreyBackfillRow(job as any, r.detail ?? null,
    { hasPersistedDocket: (r.dockets ?? 0) > 0, hasInvoiceObligation: false });
  const d = row.disposition ?? "(none)";
  tally[d] = (tally[d] ?? 0) + 1;
  out.push({ job_number: r.job_number, disposition: d, storeys: row.storeys ?? null, reason: row.reason ?? row.refusal ?? null });
}
