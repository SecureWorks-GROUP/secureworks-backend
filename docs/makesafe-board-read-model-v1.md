...
Access to the endpoint itself is a role allow-list (`MAKESAFE_TRADE_PROJECTION_ROLES`: `admin`, `owner`, `ops_manager`, `crew`, `estimator`, `installer`, `lead_installer`, `sales`); any other role is refused with 403. **What a caller SEES is a separate question**, decided since Captain ruling 2026-09-24 by `users.trade_sees_all_jobs` and `users.managed_verticals` alone — never by role, and never a special case for a particular role or a particular managed-vertical string. See `docs/trade-all-means-all-v1.md`'s 2026-09-24 addendum for the full rule table.

- Ordinary `crew`, `estimator`, `installer`, and `lead_installer` profiles (and any other allow-listed role with no see-everything flag and no `makesafe` managed vertical) receive only jobs carrying their own assignment. Their assignment array is also reduced to their own rows. The server scopes the read to their assigned job ids before building the canonical model, so an allocated-only caller never loads or receives full make-safe history.
- A profile managing the `makesafe` vertical (Hugo), or a caller with `users.trade_sees_all_jobs = true` (Shaun, Marnin, Jan, Esther), receives all make-safes including complete, archived and cancelled cards, and `can_allocate` is true. This is role-independent: a `crew`-role user with `managed_verticals: ['makesafe']` gets the identical answer to an `ops_manager`-role user with the same list.
- **Retired 2026-09-24**, because nothing live depended on them: the `makesafe_view` / `makesafe_readonly` view-only managed-vertical special case (no production row ever carried either value — this was Jan's entry, and Jan is now on the see-everything list directly), and the `fencing_view_only` special case for a fencing-managed or `sales`-role profile (Khairo; no production user held `role: 'sales'`, and `trade.html` never read `fencing_view_only`). A fencing-only manager (any role) now gets plain `allocated_only` on this endpoint, exactly like every other category they don't manage — `permissions.fencing_view_only` stays `false` in the payload shape only, so an existing reader of that key never sees it disappear.
