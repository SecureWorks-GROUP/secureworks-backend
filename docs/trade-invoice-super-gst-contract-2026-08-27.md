...
Labour lines stay at the work amounts (the job/work-order nets the trade is
billing). Super is a **minus** line so the bill total equals cash payable to the
trade (`trade_payable` / OSCO payout). Super is paid to the fund separately and
is never added on top of labour.

The super line description is human wording:

`Superannuation Guarantee (12.00%) withheld from <trade> — paid to the super fund separately`
