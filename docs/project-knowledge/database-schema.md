...
- **job_assignments** — scheduling. Links jobs to users with dates, times, crew, status, started_at, completed_at. Also carries **is_ghost** (`boolean NOT NULL DEFAULT false`) — live drift, in no repo migration. Ghost rows (in practice always `role:'observer'`) mirror a job onto the ops manager's list. Since 2026-09-17 they are auto-mirrored from every genuine dated crew assignment and kept in step with it on reschedule, cancel and delete by `supabase/functions/ops-api/ghost_observer_mirror.ts` (write-side contract in AGENTS.md, "Every genuine, dated crew assignment auto-mirrors a ghost"); hand-placed ghosts that predate it were never rescheduled and can still carry stale dates. Note `job_assignments_job_user_date_key` is UNIQUE(job_id, user_id, scheduled_date) with no `is_ghost`/`status` exemption. Either way the `calendar_events` view and every `my_jobs` feed read exclude them with `is_ghost = false` (see `docs/evidence/trade-feed-ghost-row-source-exclusion-2026-08-06.md`), and `ops-api?action=pipeline` excludes them in code from its `next_scheduled_date`/`last_scheduled_date` visit dates while deliberately keeping them in the frozen `first_scheduled_date`/`assignment_count` (contract: `docs/project-knowledge/ops-dashboard.md`).
