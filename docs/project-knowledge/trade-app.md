...
- Client contact is narrower than job access. Only a caller with their own
  non-cancelled assignment receives `contact`; Tier 1 and Tier 2 receive the same
  projection. It contains only `client_name`, `job_number`, `phone`, `address`,
  and server-built `call` / `navigate` actions. A missing phone produces an
  unavailable call action with `No client phone on file`. Unassigned
  dispatcher, managed-vertical, and open-MakeSafe viewers receive `contact: null`.
  Raw client contact/address fields are absent from `job`, and no client email,
  text/email action, or messaging permission is part of this entitlement.
...
| `trade_job_detail` | GET | Full job view: docs, media, notes, POs, crew, work order, and the current-cycle report. Client contact is a separate active-assignment entitlement: an assigned Tier 1 or Tier 2 caller receives `contact` with only client name, job number, phone, address, and server-built call/navigation actions; missing phone is explicitly unavailable. An otherwise-authorized but unassigned viewer receives `contact: null`. Raw client contact/address fields and client email are absent from `job`; the contact block grants no text/email action or messaging permission. `documents` is filtered server-side to `visible_to_trades = true` (it used to ship every row and leave the decision to the client — 38 quote PDFs flagged false were reaching installers). Additive keys: `scopeSummary` (derived from `scope_json` by the same `buildScopeSummaryLine` ops uses for invoice descriptions), `workOrders` (every live work order — `workOrder` remains the newest one), `workOrderDocuments`, `crew[].is_lead`/`crew[].name`, `leadInstaller` (null when nobody is designated), and `job_identity` (typed WO group, typed PO job grain, and canonical key; no metadata or render-time filename parsing). MakeSafe detail also carries the server-owned completion handoff (`physical`, `own_template`, `builder_portal`, or `unknown`) and a flag-only billing-review warning; it never carries invoice identifiers, amounts, or disposition options. Contract: `docs/evidence/trade-crew-visibility-lead-2026-08-03.md` |
| `set_job_lead` | POST | Name (or clear, with `clear:true`) the lead installer on a job's crew, by `assignmentId` or `userId`. Gated by the SAME `assertAssignmentMutationAuthz` that gates `create_assignment`: dashboard/service key, dispatcher, or a manager of that job's vertical. An ordinary assigned installer is refused, including on their own row. At most one lead per job is enforced by `uq_job_assignments_one_lead`, not by application code |
