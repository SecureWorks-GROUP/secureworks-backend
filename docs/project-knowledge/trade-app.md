...
| `trade_job_detail` | GET | Full job view: client, docs, media, notes, POs, crew, work order, and the current-cycle report. `documents` is tier-dependent: office and a manager of the job's vertical get the full document set, an allocated trade (and the open-MakeSafe tier) get `visible_to_trades = true` rows minus every `quote`/`invoice` type whatever the flag says (quote **numbers** on remaining rows stay). `job.scope_json` is quote-redacted server-side for those tiers (`redactTradeScopeQuote`): narrative + quote numbers stay, every money/price field is stripped (TRD-4). `media` / `currentCycleMedia` are the same trade list — current-cycle rows plus **all job videos** (videos are fetched on a separate uncapped lane so the 200-row media page cannot hide them), with scope walkthroughs promoted from `scope_json` (`job.scopeMedia` or top-level `scopeMedia`) into `job_media` when a public **https** URL already exists (`http://` and `data:video` are ignored on this read). The response carries the resolved `access_tier` and `quote_visible`, so the client never re-derives authority. Tier contract: `docs/evidence/trade-access-model-2026-08-17.md`. TRD-4 UX contract: `docs/evidence/trd-4-trade-quote-video-contract-2026-09-06.md`. Additive keys: `scopeSummary` (derived from `scope_json` by the same `buildScopeSummaryLine` ops uses for invoice descriptions), `workOrders` (every live work order — `workOrder` remains the newest one; allocated viewers get `scope_items` without rate/total/unit_price), `workOrderDocuments`, `quote_extracts` (price-free printable extract pointers for sent/accepted quotes; fetch via `trade_quote_extract`; not injected into `documents`), `crew[].is_lead`/`crew[].name`, `leadInstaller` (null when nobody is designated), `job_identity` (typed WO group, typed PO job grain, and canonical key; no metadata or render-time filename parsing), `job_family` / `vertical`, and `repair` (only for a repair-family job: `builder_work_order_number`, `builder_po_number`, `builder_claim_ref` as stored in metadata, plus `repair_stage`; `null` otherwise — 2026-09-17 addendum in `docs/trade-all-means-all-v1.md`). MakeSafe detail also carries the server-owned completion handoff (`physical`, `own_template`, `builder_portal`, or `unknown`) and a flag-only billing-review warning; it never carries invoice identifiers, amounts, or disposition options. Contract: `docs/evidence/trade-crew-visibility-lead-2026-08-03.md` |
| `trade_quote_extract` | GET | Price-free printable quote extract for a job the caller may already open (`assertAssignedOrMakesafeAccess`, same as `trade_job_detail`). Assembled from frozen `trade_pack_json` plus customer/terms snapshots. Params: `jobId` (required), optional `document_id` (quote `job_documents.id`), `format=json` (default: `{ schema, extract, html, filename }`) or `format=html` (raw printable HTML + `Content-Disposition`). No `$`, rates, or totals. Office still uses the priced quote PDF. UX print button lives in secureworks-ux. Contract: `docs/evidence/trd-6-trade-quote-extract-2026-09-06.md` |
