...
| `trade_job_detail` | GET | Full job view: client, docs, media, notes, POs, crew, work order, and the current-cycle report. `documents` is filtered server-side to `visible_to_trades = true` (it used to ship every row and leave the decision to the client — 38 quote PDFs flagged false were reaching installers). Additive keys: `scopeSummary` (derived from `scope_json` by the same `buildScopeSummaryLine` ops uses for invoice descriptions), `workOrders` (every live work order — `workOrder` remains the newest one), `workOrderDocuments`, `crew[].is_lead`/`crew[].name`, and `leadInstaller` (null when nobody is designated). MakeSafe detail also carries the server-owned completion handoff (`physical`, `own_template`, `builder_portal`, or `unknown`) and a flag-only billing-review warning; it never carries invoice identifiers, amounts, or disposition options. Contract: `docs/evidence/trade-crew-visibility-lead-2026-08-03.md` |
