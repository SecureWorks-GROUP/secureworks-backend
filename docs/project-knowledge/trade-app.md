...
| `generate_trade_invoice` | POST | Generates the holder's invoice from weekly hours and structured extras. Work-order extras persist `wo_allocated`, `wo_labour_deduction`, and cleaned `wo_labour_lines`; the server rejects a claimed WO net that does not equal allocation minus declared labour. After the holder invoice commits, each uniquely resolved named labourer receives a separate `pending_ops_review` payout invoice through the existing office approval/push flow; unnamed, unmatched, ambiguous, self-named, and fan-out failures are returned and recorded for manual handling. The response includes `wo_labour_payouts` and `wo_labour_unresolved`. |
| `trade_job_detail` | GET | Full job view: client, docs, media, notes, POs, crew, work order, and the current-cycle report. MakeSafe detail also carries the server-owned completion handoff (`physical`, `own_template`, `builder_portal`, or `unknown`) and a flag-only billing-review warning; it never carries invoice identifiers, amounts, or disposition options |
