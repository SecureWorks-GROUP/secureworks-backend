...
The database ladder binds direct references, known threads, one open contact job (a `draft` counts only when the contact has no live open job, since `20260921140000`), or one open job of the line. Multiple candidates become `pending_luna`; no match stays in `admin_bucket`. Identity is GHL contact id only, never a name. No human queue. Empty/pointer-only and automated rows remain stored but are not extracted. Automated rule inputs are `payload.automated`, `payload.auto_submitted`, existing automated status, and system/audit channels; B5 owns mail-specific rules. The booking-lane hangar is `ensure_booking_draft_job` in that same migration: callable, idempotent on `ghl_contact_id`, no trigger or backfill.
