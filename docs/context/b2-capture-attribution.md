...
The bounded rotating bucket rerun has no source-age cutoff. A 180-day-old bucket event can bind when a matching job appears, but a dormant historical row still cannot activate extraction without a new signal. Explicit source IDs are retained in `metadata.source_job_binding` while attribution is off and validated against jobs on resume; hints stay separate and cannot take this direct recovery path.
