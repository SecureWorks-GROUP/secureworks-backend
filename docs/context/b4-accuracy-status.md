...
- `context_accuracy_publish(p_week_start date) -> jsonb`: recomputes exact verdict totals and separate current coverage. Fully judged undersized samples report true/actual, requested 40, missing, and sample_sufficient=false. Partial reviews report no accuracy ratio. Two consecutive calendar weeks each with an observed breach stop extraction: more than two wrong-job facts is sufficient even with an incomplete review; percentage-based breach requires all intended 40 judged and fewer than 36 true. Different breach types across the two weeks count. Both adjacent calendar pairs are checked when a week changes, so late or reverse-order reviews cannot bypass the stop. A missing week breaks the sequence. Later corrections never automatically re-enable extraction or erase its pending alert. Any explicit human invented-payment false verdict stops immediately, including a finding made after publication. Nothing automatically re-enables extraction.
...
- `context_coverage() -> jsonb`: current facts versus open jobs, and separately open authorised receivable invoices; unlinked invoices remain a distinct unknown. `no_current_fact` counts linked records without current facts; `evidence_without_current_fact` identifies those with stored source evidence, including pending extraction or expired facts; `no_evidence_yet` is restricted to those with neither. Unlinked invoices remain separate. These coverage measures are not accuracy judgments.
...
Review-fix contracts additionally cover reverse-order partial wrong-job pairs, a reverse-order mixed full-40 percentage/wrong-job pair, corrections that preserve the stop and pending alert, and separate no-source/pending-source/expired-fact coverage for jobs and linked invoices. These additive cases await the combined registered-stack run; their presence alone is not execution proof.
