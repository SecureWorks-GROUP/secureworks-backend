...
`mapping.outcome` is one of `created`, `deliberate_repeat_created`, `existing_opportunity_reused`, `existing_contact_job_reused`, `concurrent_request_reused`, `completed_opportunity_reused`, or `idempotent_replay`. `canonicalOutcome` preserves the ledger's original outcome when the current transport is a replay/race join.

`concurrent_request_reused` and `completed_opportunity_reused` are distinct and must not be conflated. `concurrent_request_reused` means a different in-flight request for the same identity won the race and this caller joined it. `completed_opportunity_reused` means this requestId re-entered a lead that a prior mint had already completed against, possibly long ago: nothing was concurrent, and the response is that completed canonical job. Genuinely new work on a completed opportunity is out of scope for this command and requires a separate explicit flow.
