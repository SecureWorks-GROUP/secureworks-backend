...
`proof_plan_send` resolves the full route without sending. The harness checks it before calling `proof_execute_send`. Any other address, any extra To address, or any Cc or Bcc stops the run before transport.
...
Cleanup runs in a final stage even after an earlier failure. It compares the server plan with the local creation ledger, executes marker-scoped cleanup, then independently reads back:
...
| `ses_captain_delivery_v1` | `proof_plan_send`, `proof_execute_send`, `proof_verify_delivery` | Deliver Shaun safe-today and Marnin run-with-care evidence only to the Captain inbox, then read back the exact message marker plus `approved_pack` and `draft_invoice` attachments. |
