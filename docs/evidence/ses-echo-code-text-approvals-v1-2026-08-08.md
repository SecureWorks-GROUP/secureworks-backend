...
6. The separate SEND IT route and `ses_channel_send_it.ts` — unchanged and not
   called by echo-code approval.
...
in refusal facts. Verification consumes the request before returning a pass or
fail, including sender, message, and code mismatches.
...
- `deno test --no-check supabase/functions/ops-api/ses_echo_code_approval_test.ts`
  — 9 passed, including all watched attack failures and the relay-authority
  refusal.
