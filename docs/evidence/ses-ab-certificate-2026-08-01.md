...
certified FLOOR plus a partition rule, and the checks assert what those numbers
were protecting:
...
PASS phase_a  a2_emails_without_case  no persisted SES email lacks a case-source row
       ...
       observed 0
PASS phase_a  a3_identity_key_census  identity keys are fully accounted as live-job / exception-only / synthetic
       expected {"unaccounted":0,"partition_complete":true,"keys_lost":0,"live_job_regression":0,"synthetic":4}
       observed {"unaccounted":0,"partition_complete":true,"keys_lost":0,"live_job_regression":0,"synthetic":4}
       note     census invariant, not a pinned count: certified floor 534 total / 157 live-job / 373 exception-only / 4 synthetic; live now 534 total (+0) / 160 live-job (+3) / 370 exception-only / 4 synthetic. Captain rule, 2026-08-01 addendum: the identity-key and live-case counts above are a certified FLOOR and a partition rule, not an expected observation. Ordinary intake keeps minting keys and keeps promoting exception cases to confirmed_live_job, so a3/a4 assert the invariants those numbers protect - every key lands in exactly one of live-job / exception-only / synthetic, no key or live case is destroyed (no negative growth from the floor), promotion only runs exception -> live, the synthetic set stays closed, and a confirmed_live_job case always has its job - and report growth above the floor instead of failing it. Pinning the exact counts is what made the certificate fail on healthy +3 live-job intake growth on 2026-08-01. Do not re-pin them; if a count must move DOWN, that is a real defect and the invariant is meant to catch it. The brief's pre-recovery census was 533 total / 156 live-job; The task brief's 533/156 is the pre-recovery snapshot. PR 460's captain-ruled exact rescan promoted one previously key-less exception case (reason_code adapter_parse_failure, therefore no wo_po_identity_key) to confirmed_live_job with the newly extracted key wo:MLB-27309/po:PO-57445. Exception-only stays 373 and synthetic stays 4, which is only possible if the promoted case carried no key before: had it already held one, exception-only would now read 372.
PASS phase_a  a6_keys_with_two_live_jobs  no identity key carries two live case-bound jobs
       expected 0
       observed 0
PASS phase_a  a4_live_case_without_job  no confirmed_live_job case is missing its job
       expected {"live_case_without_job":0,"live_cases_regression":0}
       observed {"live_case_without_job":0,"live_cases_regression":0}
       note     certified floor 158 live cases; live now 161 (+3 post-baseline intake growth).
PASS phase_a  a5_dangling_job_ids  no case, correction target or source link points at a missing job
       expected {"case_job":0,"target_job":0,"link_job":0}
       observed {"case_job":0,"target_job":0,"link_job":0}
PASS phase_a  a7_captain_creation_roof_report  SWMS-261123 exists as a roof_report card with lineage to MLB-27309
